...
def _len_note(b, text):
    """Прирост узла СПРАВКОЙ, а не приговором: длина известна — назовём её, нет — промолчим.

    С 17.08.2026 длина вердикта НЕ РЕШАЕТ (см. `_judge_brain`), но и не пропадает: она приехала
    в фактах, и молчать о том, что знаешь, — это тот же «нуль по неразбору» с другой стороны."""
    try:
        before = int(b.get("len_before"))
    except (TypeError, ValueError):
        return ""
    now = int(b.get("len") if b.get("len") is not None else len(text))
    if now > before:
        return ", вырос %d → %d" % (before, now)
    return ", длина прежняя (%d → %d)" % (before, now)


def _judge_brain(parts, facts):
    """Узел мозга: ПРОЧИТАН по названному ключу И СОДЕРЖИТ названную подстроку.

    ДОКАЗАТЕЛЬСТВО — НАЙДЕННАЯ ПОДСТРОКА (решение Штаба 17.08.2026). Прежняя длина узла больше
    не обязательна: адрес несёт ключ и подстроку, и этого довольно.

    ЗАБОР ЧЕСТЕРТОНА НАЗВАН, А НЕ СНЯТ МОЛЧА. Требование «И ВЫРОС» защищало ровно от одного:
    подстрока могла лежать в узле и ДО шага, и тогда «содержит» доказывает не работу, а
    совпадение. ЭТУ ЗАЩИТУ МЫ ТЕРЯЕМ — сказано прямо. Взамен не поставлено ничего машинного, и
    вот почему это всё-таки лучше: прежней длины НЕ ПИСАЛ НИКТО (производителей ноль, разбор
    17.08 §4в), поэтому забор стоял не между «работа» и «совпадение», а между «неизвестно» и
    ВСЕМ ОСТАЛЬНЫМ — прибор молчал обо всех узлах одинаково, ни одного совпадения при этом не
    поймав. Молчащий прибор не защищает; он только выглядит строгим.

    ЧТО ОСТАЁТСЯ ВМЕСТО НЕЁ — ДИСЦИПЛИНА АДРЕСА: подстрока обязана быть тем, чего до шага быть
    не могло (свой хеш, своё имя артефакта, своя дата-время). Это ответственность того, кто
    пишет адрес, и она названа ЗДЕСЬ именно затем, чтобы её нельзя было принять за машинную.

    Замок против ложного зелёного не тронут с другой стороны: узел НЕ ПРОЧИТАН → НЕИЗВЕСТНО
    (и причина называется дословно, вплоть до «руки сломаны»), подстроки НЕТ → НЕ ДОКАЗАН."""
    ...
    if parts["expect"] not in text:
        return _v(UNPROVEN, "узел «%s» названного не содержит (прочитано %d знаков)"
                  % (parts["key"], len(text)))
    return _v(PROVEN, "узел «%s» содержит названное%s" % (parts["key"], _len_note(b, text)))
