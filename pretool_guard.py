...
(б) ДАННЫЕ ≠ КОМАНДА: красное слово внутри ПОИСКОВОГО ШАБЛОНА grep/rg/sed/awk (аргумент
    ...
    Тот же класс с 05.08.2026 — ИМЯ СЕКРЕТА В АРГУМЕНТЕ: улика «файл секретов» это ПУТЬ, а не
    написание (_env_path_arg). Элемент argv доезжает до скрипта ЦЕЛИКОМ, поэтому токен-путь
...
(д) ПРАВИЛО ИСПОЛНЯЮЩЕЙ ПОЗИЦИИ (04.08.2026) — ОДНО правило вместо разряда за разрядом. Краснит
    имя, стоящее там, где его ИСПОЛНИТ разборщик: разобранный вызов/имя кода/литерал-селектор при
    наличии канала (python-тело, AST) либо слово-команда и тело подстановки (оболочка, разбор цепи).
    Закрыты ДВЕ ЖИВЫЕ ТЕЧИ, обе решают про разрешения: имя операции в ПУТИ скрипта краснило команду
    с ЧИСТЫМ телом (тело у отслеживаемого исходника гард не читает вовсе), и SQL-глагол в
    ПЕЧАТАЕМОЙ строке рождал карточку с ОБЪЕКТОМ, вытянутым из той же прозы.
    ГРАНИЦА НАЗВАНА ЧЕСТНО: строковый литерал НЕ-денежной операции (докстринг, печатаемая строка,
    список слов) краснит ПО-ПРЕЖНЕМУ — так записано решением 02.08.2026 («докстринг остаётся
    красным намеренно: строка = возможное действие», tests/test_root_a_vps.py, остатки 117/135 в
    tests/test_money_action.py). Распространить правило туда — снять существующее красное в трёх
    классах сразу; это развилка владельца, а не правка гарда.
    Жёсткие классы (процессы, файл секретов) и SQL текста САМОЙ команды судятся по ПОЛНОМУ скану —
    правило их не касается. Сомнение по-прежнему решается краснее: нет разбора (heredoc/stdin/
    нечитаемый .py/кривые кавычки) → СЛЕПОЙ режим, прежняя подстрока байт-в-байт.
    Регресс: tests/test_exec_position.py.
...
# ПАМЯТЬ ЗАХОДА (05.08.2026): «эту операцию владелец в этом заходе уже разрешил». Импорт
# FAIL-SAFE — модуля нет/сломан → память недоступна, гард спрашивает как спрашивал.
try:
    import repeat_ask
except Exception:
    repeat_ask = None
        ...
        if isinstance(node, ast.Call):
            ...
            for a in list(node.args) + [k.value for k in node.keywords]:
                ...
                if name in _SQL_EXEC_CALLS:
                    sql_args += _str_consts(a)   # SQL, отданный ИСПОЛНЯЮЩЕМУ вызову (не печати)
...
# ── ПРАВИЛО ИСПОЛНЯЮЩЕЙ ПОЗИЦИИ — ОДНО НА ВСЕ ИМЕНА (04.08.2026) ──────────────────────────────
# Асимметрия выше («у прочих операций сомнение решается краснее и это терпимо») держалась ровно до
# тех пор, пока «терпимо» оставалось правдой. Разбор мест, где СЛОВО — единственный признак, дал
# две живые течи ВНЕ денег, обе решают про разрешения:
#   * имя операции в ИМЕНИ ФАЙЛА краснило команду с ЧИСТЫМ телом (`python3 …/create_booking_*.py`):
#     тело гард у отслеживаемого исходника не читает вовсе (trust-by-origin), и красное рождал
#     ПУТЬ — то есть ровно то, что разборщик не исполняет;
#   * SQL-глагол с именем БД в ПЕЧАТАЕМОЙ строке рождал карточку, а ОБЪЕКТ её («БД …») тянулся из
#     той же прозы: карточка про запись, за которой никакой записи нет.
# Поэтому правило ОДНО и общее: КРАСНИТ ИМЯ, СТОЯЩЕЕ В ИСПОЛНЯЮЩЕЙ ПОЗИЦИИ — там, где его исполнит
# разборщик. Двух списков нет и не заводится: у python-тела исполняющую позицию называет AST
# (_name_is_action), у команды оболочки — разбор цепи (_exec_scan), у SQL — вызов курсора
# (_sql_is_executed). ГДЕ ПРАВИЛО ОСТАНОВЛЕНО, сказано в _body_has: строковый литерал НЕ-денежной
# операции краснит по-прежнему, потому что так РЕШЕНО 02.08.2026 и записано голденом со словом
# «намеренно». Снять это — развилка владельца; гард своей властью записанное решение не отменяет.
# Направление сомнения прежнее: разбора нет (heredoc/stdin/нечитаемый .py/кривые кавычки) → СЛЕПОЙ
# режим, прежняя подстрока БАЙТ-В-БАЙТ, то есть краснее. Жёсткие классы (процессы, файл секретов) и
# SQL в тексте САМОЙ команды судятся по ПОЛНОМУ скану — они этим правилом не ослабляются.
_SQL_EXEC_CALLS = frozenset(("execute", "executemany", "executescript"))


def _name_is_action(tok, raw, view, blind=False):
    """Имя ДЕЙСТВУЕТ (красное) или лишь УПОМЯНУТО (не красное)? Единая проверка для ВСЕХ имён
    операций: разобранный вызов → имя кода → литерал-селектор при наличии канала."""
...
# Прежнее имя денежной проверки: правило стало общим, контракт зовущих (тесты 02.08) не ломаем.
_money_is_action = _name_is_action


def _sql_is_executed(blob, view):
    """SQL-ЗАПИСЬ стоит в ИСПОЛНЯЮЩЕЙ позиции? Разбора нет → прежняя подстрока по всему тексту
    (fail-closed, байт-в-байт). Разбор есть → судим ТОЛЬКО строки, отданные исполняющему вызову
    курсора (execute/executemany/executescript): печать, докстринг и комментарий SQL не исполняют.
    Текст САМОЙ команды здесь не теряется — его проверил шаг 1б по скан-представлению."""
    if view is None:
        return bool(_SQLITE_WRITE.search(blob or ""))
    return any(_SQLITE_WRITE.search(s or "") for s in (view.get("sql_args") or ()))
...
_ENV_BASENAME = re.compile(r"^\.env[\w.\-]*$")   # базовое имя файла секретов, занимающее токен ЦЕЛИКОМ


def _env_path_arg(tok):
    """Аргумент .py-скрипта — ОБРАЩЕНИЕ к файлу секретов? Судим по ДЕЙСТВИЮ, а не по написанию.

    КЛАСС (05.08.2026, фикс 2 из 3 «граница гейта»). argv .py-скрипта вырезается из скана как
    ДАННЫЕ с 24.07 (_strip_script_cli_args), но токен, где ВСТРЕЧАЕТСЯ написание `.env`,
    удерживался под сканом «операндом-уликой» — и жёсткий блок вставал на ИМЯ СЕКРЕТА В
    АРГУМЕНТЕ, а не на чтение секрета. ЗАМЕР (транскрипты, 336 ч, 6197 живых команд): 29
    жёстких блоков, из них 5 ложных — ВСЕ журнальные строки `cclog.py`, где имя названо в
    прозе отчёта («запрет cat/grep .env классом в CLAUDE.md»). Тот же класс и то же лечение,
    что у текста СВОЕГО КОММИТА, где он закрыт раньше (payload -m — bd5d516, heredoc — a77c14e):
    в прозе имя секрета уже не краснит, в аргументе оставалось живым.

    РАЗЛИЧИТЕЛЬ — не эвристика, а СЕМАНТИКА argv: элемент argv доезжает до скрипта ЦЕЛИКОМ.
    Токен, который САМ ЕСТЬ путь (`/root/app/.env`, `.env.local`, `--config=/root/app/.env`),
    скрипт может открыть — это обращение, и оно остаётся красным ВСЕГДА. Токен-ПРЕДЛОЖЕНИЕ
    («…запрет cat/grep .env классом…») не откроется ничем: это текст, а не имя файла. Поэтому
    решает пробел ВНУТРИ токена, а не длина, язык или знаки препинания вокруг имени.

    НАПРАВЛЕНИЕ СОМНЕНИЯ прежнее — в красное: имени нет вовсе → улики нет; имя есть, а путь
    разобрать не удалось → токен остаётся под сканом. ЧЕСТНЫЙ ПРЕДЕЛ НАЗВАН: скрипт, который
    САМ разберёт свою строку и откроет из неё кусок, этой проверкой не ловится — но и не
    ловился, а токен с признаком исполнения ($(…)/`…`/system(…)) держится отдельной уликой в
    том же условии и под сканом остаётся. Линия старше этой правки: payload `git -m`
    вырезается ЦЕЛИКОМ с 12.07, включая `-m "cat /root/app/.env"`."""
    t = (tok or "").strip().strip("'\"")
    if not t or not _ENV_FILE.search(t):
        return False                       # написания нет вовсе → и улики нет
    if t.startswith("-") and "=" in t:      # флаговая форма --config=<путь>: значение — тот же операнд
        t = t.split("=", 1)[1].strip().strip("'\"")
    if not t or re.search(r"\s", t):
        return False                       # пробел ВНУТРИ токена → это ТЕКСТ, а не путь
    try:
        return bool(_ENV_BASENAME.match(os.path.basename(t)))
    except Exception:
        return True                        # путь не разобрался → под скан (fail-closed)
...
_P_MSGID = [r"\bmsg_id\s*[=:]\s*['\"]?([\w:.\-]{2,40})",
            r"['\"]msg_id['\"]\s*:\s*['\"]([\w:.\-]{2,40})"]
# ── ПОЛЯ ОСТАЛЬНЫХ ДВЕРЕЙ ПОДТВЕРЖДЕНИЯ (26.08.2026, разбор 2026-08-26-guard-deadlock.md) ──────
...
#   set_fleet_oil ReadFleet:266 · set_fleet_service :423   — объект `number`, знаком с 28.07;
#   edit_event    BotData:631   — `msg_id`;   service_undo ServiceUndo:243 — `act`;
#   set_caps      QuotePrice:316 — `caps`;    toggle_cap   QuotePrice:376 — `model` + `on`.
# Имя первых двух стоит в RED_TOKEN_HIT, поэтому у них СВОЙ хит и своя ветка объекта. У остальных
# четырёх имени в словаре нет — хит схлопывается в общий `confirmed`, а его словарь объектов знал
# РОВНО байк и клиента, то есть поля двух дверей из шести. Объект выходил пустым, `card_gate` гасил
# карточку (правило `bool(obj)` верное и НЕ тронуто), а маркер демону и пуш владельцу в этой ветке
# НЕДОСТИЖИМЫ — красное умирало молча, строкой в журнал, которого не читает ни один механизм.
# ЖИВОЙ СЛУЧАЙ: заход 25.08 упёрся в дверь отмены записи ТО (`_post('service_undo', act=…,
# confirmed=True)`) и не смог даже СПРОСИТЬ — ни исполнить, ни позвать владельца.
# РАСШИРЕНИЕ ОДНОСТОРОННЕЕ: имена только ДОБАВЛЯЮТСЯ к объекту КАРТОЧКИ и приписываются ПОСЛЕ
# прежних, поэтому прежняя подпись остаётся началом новой — «расширение, а не подмена» видно
# глазом и проверяется механически (tests/test_confirmed_doors.py, секция 4).
_P_ACT = [r"\bact\s*[=:]\s*['\"]([\w.:\-]{2,40})",
          r"['\"]act['\"]\s*:\s*['\"]([\w.:\-]{2,40})"]
_P_MODEL = [r"\bmodel\s*[=:]\s*['\"]([^\"']{2,40})",
            r"['\"]model['\"]\s*:\s*['\"]([^\"']{2,40})"]
# Состояние капа — ЧИСЛОМ карточки (`on` не объект, а то, ЧЕМ значение станет: та же мысль, что у
# ветки edit_event). Значение берём только НАЗВАННОЕ (кавычки либо булево), иначе двухбуквенное имя
# ловило бы что угодно; на рождение карточки это не влияет никак — гейт судит ОБЪЕКТ.
_P_ON = [r"\bon\s*[=:]\s*['\"](on|off|да|нет|true|false)['\"]",
         r"\bon\s*[=:]\s*(True|False)\b",
         r"['\"]on['\"]\s*:\s*['\"]?(on|off|да|нет|true|false|True|False)"]
# `caps` несёт СПИСОК блоков, а не одно значение: объектом служит сам факт блока капов, а конкретную
# модель, если она в нём названа, добавит _P_MODEL. Готовый компилированный признак (идиома _P_DIGIT)
# — извлекать тут нечего, вопрос только «назван ли блок».
_P_CAPS = re.compile(r"""(?:\bcaps\s*[=:]|['"]caps['"]\s*:)\s*[\[{]""", re.I)
...
def _detail_parts(hit, blob, doors=True):
    ...
    `doors` (26.08.2026) — читать ли поля ОСТАЛЬНЫХ дверей подтверждения (см. блок над _P_ACT).
    Значение по умолчанию ИСТИННО, поэтому контракт `card_min(hit, blob)`, на который опираются
    тесты обеих полос, не изменился ни на символ и третьего параметра не получил. `False` приходит
    РОВНО из одного места — `_entity_blocktype`, и почему, сказано там."""
    obj, num = [], []
    if hit in _HITS_FLEET:
        ...
        if c:
            obj.append("клиент " + c)
        if doors:
            # Поля ОСТАЛЬНЫХ дверей подтверждения — см. блок над _P_ACT. Стоят ПОСЛЕ прежних
            # намеренно: прежний объект остаётся НАЧАЛОМ нового. `doors=False` приходит с одного
            # места — от забора живой сущности, который обязан судить ровно тем же, чем судил.
            a_ = _find(_P_ACT, blob)
            if a_:
                obj.append("акт " + a_)
            e_ = _find(_P_MSGID, blob)
            if e_:
                obj.append("событие " + e_)
            if _P_CAPS.search(blob or ""):
                obj.append("блок капов")
...
def _entity_blocktype(hit, blob):
    ...
    if hit in _ENTITY_HITS_LIVE and not _blob_has_test_entity(blob):
        ...
        # ЗАБОР СУДИТ РОВНО ТЕМ ЖЕ, ЧЕМ СУДИЛ (26.08.2026, `doors=False`). Словарь объектов класса
        # `confirmed` расширен именами остальных дверей подтверждения — но расширен он ДЛЯ КАРТОЧКИ.
        # Сюда новые имена не идут намеренно, и это не осторожность, а направление: здесь «объект
        # есть» означает НЕ карточку, а deny без кнопки (задача failed). Пусти я их сюда — команда,
        # которая сегодня молча уходит в журнал с решением `ask`, завтра стала бы жёстким блоком:
        # карточек не прибавилось бы, а прибавилось бы отказов, то есть ровно наоборот замыслу.
        # ЧТО ЭТИМ НЕ ОСЛАБЛЕНО: живая сущность (клиент/байк) извлекается выше и здесь не участвует
        # вовсе; для всех прочих хитов `doors` не читается ни одной веткой, поэтому решения
        # `hard`/None у них побайтно прежние. Доктрина «ТЕСТ-сущность или решение владельца» цела:
        # у дверей отмены и капов живой сущности нет ВОВСЕ, пометить её ТЕСТом нечем, и жёсткий
        # блок означал бы вечный отказ без единого способа спросить.
        obj, _num = _detail_parts(hit, blob, doors=False)
        if ", ".join(obj).strip():
            return "hard"
    return None
...
def _card(hit, blob="", test=False, cmd="", count=1, repeat=0):
    ...
    rep = ("Повтор: ×%d — та же команда в этой задаче (карточка обновлена, новых не шлю)\n" % count) \
        if count > 1 else ""
    # ПОВТОР ОПЕРАЦИИ ≠ ПОВТОР КОМАНДЫ (05.08.2026). Строка выше считает одинаковый ТЕКСТ, эта —
    # одну и ту же ОПЕРАЦИЮ (класс+объект+число), уже разрешённую владельцем в этом заходе. Она
    # появляется только у классов-ПЕРЕХОДОВ: там вопрос остаётся, но вторая карточка обязана
    # отличаться от первой — иначе владелец подтверждает второй факт, читая текст первого.
    again = ("Повтор операции: это %d-й раз в этом заходе — прежнее «да» относилось к первому; "
             "повтор рождает НОВЫЙ факт\n" % repeat) if repeat > 1 else ""
    return (head +
            "🔴 КРАСНОЕ\n" + rep + again +
            "Что: " + what + "\n" + cmd_line +
            "Объект: " + (obj or "—") + "\n"
...
def _repeat_seen(data, hit, obj, num):
    ...
    Объект не назван → отпечаток пуст → 0, вопрос стоит как стоял.

    FAIL-SAFE ВЕЗДЕ: нет модуля памяти, нечитаемый стор, любое исключение → 0, то есть прежнее
    поведение байт-в-байт. Память умеет только ДОБАВИТЬ вопросу пометку «повтор» либо снять
    вопрос-дубль; выдать право она не может — гард не умеет говорить «allow» вовсе."""
...
def _strip_script_cli_args(cmd):
    ...
    УЛИКА «ФАЙЛ СЕКРЕТОВ» С 05.08.2026 — ЭТО ПУТЬ, А НЕ НАПИСАНИЕ (_env_path_arg, см. там замер и
    предел): токен-ПУТЬ держится под сканом как держался, токен-ПРЕДЛОЖЕНИЕ с именем секрета в
    прозе — данные, как и весь остальной argv. `python3 dump.py /root/app/.env` блокируется
    по-прежнему; `cclog.py DONE "…правил .env…"` больше не выдаётся за обращение к секретам.
    Прочие улики (.db/SQL-write/исполнение) правкой НЕ тронуты — это отдельные классы.
    ...
    for i, t in enumerate(toks):
        ...
        keep = [a for a in tail if _env_path_arg(a) or ".db" in a
...
_PATH_OPERAND = re.compile(
    r"[/\\]|\.(?:py|sh|md|txt|json|log|ya?ml|csv|tsv|html?|js|ts|ini|cfg|conf|toml|bak)$", re.I)


def _data_operands(toks, idx):
    """Токены сегмента, которых разборщик НЕ ИСПОЛНЯЕТ: пути и имена файлов-операндов. Слово-команда
    (idx) и флаги не трогаются — их шелл исполняет и читает. ОПЕРАНДЫ-УЛИКИ остаются под сканом той
    же линией, что в _strip_script_cli_args: файл секретов, .db, SQL-write и токен с признаком
    исполнения ($(…)/`…`/system(…)) — иначе ослабли бы жёсткий блок и класс БД."""
    out = []
    for k, t in enumerate(toks):
        ...
        if not t or t.startswith("-"):
            continue
        if (_EXEC_IN_PATTERN.search(t) or _ENV_FILE.search(t) or ".db" in t
                or _SQLITE_WRITE.search(t)):
            continue
        if _PATH_OPERAND.search(t):
            out.append(t)
    return out


def _exec_scan(cmd):
    """Скан-представление в ИСПОЛНЯЮЩЕЙ позиции: то же, что _scan, минус путь-операнды.
    Употребляется ТОЛЬКО для имён операций (RED_TOKENS): жёсткие классы и SQL текста команды
    судятся по ПОЛНОМУ скану. Сбой разбора → сырая команда (fail-safe: краснит охотнее)."""
    try:
        parts = []
        for toks, text in _units(cmd):
            parts.append(_mask(text, _data_operands(toks, _cmd_index(toks))))
        out = " ".join(parts)
        return out if out.strip() else cmd
    except Exception:
        return cmd
...
def _analyze(cmd, cwd, scan=None):
    ...
    xscan = _exec_scan(cmd)          # то же представление МИНУС путь-операнды (правило позиции)
    ...
    if _is_trusted_test(cmd, cwd):
        return "green", "", cmd
    # 1) БЫСТРОГО ГРЕПА ПО КОМАНДЕ БОЛЬШЕ НЕТ (04.08.2026, правило исполняющей позиции). Здесь стоял
    #    `tok in scan` для всех имён, кроме денежных, и ровно он краснил имя операции, стоящее в ПУТИ
    #    скрипта, и имя внутри инлайн-строки, которую разбор признал бы прозой. Теперь ВСЕ имена
    #    судятся ОДНИМ правилом ПОСЛЕ разбора тел (цикл ниже), а что разбору не досталось —
    #    возвращается туда же подстрокой через слепой режим, то есть не тише прежнего.
    # 1а) confirmed=ИСТИНА в САМОЙ команде (02.08.2026) ПЕРЕЕХАЛ ВНИЗ, за цикл имён (04.08.2026).
    #     Причина названа числом: пока быстрый греп стоял выше, конкретная операция выигрывала у
    #     обобщённого признака по порядку RED_TOKEN_HIT. Правило исполняющей позиции убрало греп —
    #     и `confirmed` начал перехватывать карточку у set_fleet_oil (голден test_guard_cli_args).
    #     Обобщённый признак обязан идти ПОСЛЕ конкретных: иначе владелец видит «подтверждение
    #     записи» вместо имени операции, которую подтверждает.
    ...
    try:
        ...
        # Кривое квотирование → ambiguous (в3: defer). ВСЕ ИМЕНА ЗДЕСЬ ДОБИРАЮТСЯ ПОДСТРОКОЙ ПО
        # ПОЛНОМУ скану: разбора тел не будет вовсе, а правило исполняющей позиции рассчитано именно
        # на разбор — без этой ветки команда с незакрытой кавычкой стала бы тише, чем была.
    ...
    set_code_view(view)
    # ОДИН цикл на ВСЕ имена (04.08.2026): подстрока берётся по ИСПОЛНЯЮЩЕМУ представлению (argv
    # скрипта, шаблон поиска и ПУТЬ-ОПЕРАНД — данные), а слепым считается не только несобранный
    # разбор (amb), но и имя, произнесённое ВНЕ разобранных тел: до имени скрипта, в `-m`-модуле,
    # в heredoc. Там разбирать нечего, и молчать нельзя — это и есть регресс `<OP> перед .py`.
    seen = xscan + "\n" + content
    for tok in RED_TOKENS:
        outside = tok in xscan and tok not in content
    ...
    # 04.08.2026: глагол судится по ИСПОЛНЯЮЩЕЙ позиции — строка, отданная курсору, а не напечатанная
    # (см. _sql_is_executed). Текст самой команды проверен шагом 1б по полному скану и не ослаблен.
    if _sql_is_executed(blob, view) and _sql_write_is_foreign(blob):
...
def main():
    ...
    obj, num = card_min(hit, blob)
    # ── ОДНА ОПЕРАЦИЯ НЕ СПРАШИВАЕТСЯ ДВАЖДЫ (05.08.2026) ──────────────────────────────────
    # Стоит ПОСЛЕ жёсткого блока намеренно: тот уже вышел выше (deny без кнопки), и память
    # захода до него не доходит — снять жёсткое ею нельзя ни при каком состоянии стора.
    # ПРИСВАИВАНИЕ (мир СТАНЕТ таким) — повтор подтверждает то же состояние, решать нечего:
    # вопрос снимаем через _defer(), то есть «возражений нет», а не «разрешаю» — дальше
