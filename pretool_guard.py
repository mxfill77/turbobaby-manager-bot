...
def marker_has_object(hit, card):
    ...
    ВНИМАНИЕ (28.07.2026): проверка идёт по ТЕКСТУ готовой карточки и потому близорука — цифра
    в шаблоне действия («Лист1») считается объектом. ПЕРВАЯ линия теперь выше: card_min() в main()
    смотрит на ИЗВЛЕЧЁННЫЕ объект/число и до этой проверки безобъектную карточку не доводит."""
...
def card_min(hit, blob=""):
    """ОБЯЗАТЕЛЬНЫЙ МИНИМУМ карточки (28.07.2026) → (ОБЪЕКТ, ЧИСЛО) строками; пусто = минимума нет.
    Доктрина: карточка красной зоны говорит ЧТО меняется, у какого ОБЪЕКТА, какое ЧИСЛО и как
    ОТКАТИТЬ. Нет объекта ИЛИ числа — подтверждать в такой карточке нечего: main() её не собирает,
    а пишет строку в журнал (решение хука при этом НЕ слабеет — ask остаётся)."""
...
def main():
    ...
    # МИНИМУМ КАРТОЧКИ (28.07.2026): нет ОБЪЕКТА или ЧИСЛА — показывать владельцу нечего и
    # подтверждать в такой карточке нечего. Вместо карточки — СТРОКА В ЖУРНАЛ (факт не теряется).
    # Гард при этом НЕ слабеет: решение остаётся ask, т.е. без «да» команда не исполнится; молчим
    # только в каналах владельца (Telegram-пуш и маркер-конверт демону).
    obj, num = card_min(hit, blob)
    if not (obj and num):
