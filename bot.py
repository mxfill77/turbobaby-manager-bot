...
async def on_startup(app: Application):
    ...
    # ЖУРНАЛ ПРИНЯТЫХ РАБОТ переживает рестарт — значит после старта он обязан СКАЗАТЬ, что в нём
    # лежит: работы, принятые до перезапуска, буфер `_PENDING_WORKS` уже не помнит вовсе. Строка
    # идёт в лог ERROR'ом (это не рутина, а невыполненное ТО живого байка) и наружу не уходит НИ
    # ОДНИМ сообщением — канал в группу тут не заводится намеренно.
    try:
        _wl_line = splinter.works_persist_startup_report()
        log.info(f"  Принятые работы без пробега: {_wl_line or 'открытых нет'}")
    except Exception:
        log.exception("  → журнал принятых работ при старте не прочитан")
