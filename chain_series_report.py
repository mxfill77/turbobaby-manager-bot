...
def _rule_since(live):
    """Момент включения правила зелёного — ПО САМОМУ ФАЙЛУ, а не по флагу этого процесса.

    Спрашивать здесь `.env` было бы неверно: сверка судит ФАЙЛ, а он говорит о себе сам —
    считался он под правилом или под прежним определением. Маркера нет (файл старше правила) →
    '' → сверка идёт БАЙТ-В-БАЙТ как прежде."""
    return str(((live or {}).get("rule") or {}).get("since") or "")


def _file_verdicts(live, since=""):
    ...
    `since` пусто → прежнее определение чистоты (им сравниваются ОБЕ стороны на общей почве:
    вердикт правила считается в момент терминала и реплею недоступен по устройству)."""
    out = {}
    for r, ch in ((live or {}).get("chains") or {}).items():
        ...
        try:
            out[int(r)] = cs.chain_verdict(c, since=since)
        except (TypeError, ValueError):
            continue
    return out
...
def verify(live, replay_chains, unread=(), windows_read=True):
    ...
    # ФАЙЛ СУДИТСЯ ТЕМ ОПРЕДЕЛЕНИЕМ, КАКИМ СЧИТАЛСЯ (маркер `rule.since` лежит в нём самом),
    # иначе сверка краснела бы на том, что счёт стал строже, — то есть на правильной работе.
    rs = _rule_since(live)
    own = cs.series([v for _r, v in sorted(_file_verdicts(live, since=rs).items())], since=rs) \
        if rs else cs.series([fv[r] for r in sorted(fv)])
    ...
    if rs:
        out.append("  правило зелёного в счёте с %s (служебных корней %s · прежних цепочек %s). "
                   "ОСЬ ПРАВИЛА РЕПЛЕЕМ НЕ СУДИТСЯ: вердикт адреса считается в момент терминала "
                   "и по снимку очереди не восстанавливается — общая почва ниже сравнивается "
                   "ПРЕЖНИМ определением, одним и тем же для обеих сторон"
                   % (rs, d.get("service"), d.get("before_rule")))
