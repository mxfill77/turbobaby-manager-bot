...
Филипп открывал терминал вслепую. Теперь на permission-ожидании хук достаёт из транскрипта сессии
ПОСЛЕДНИЙ tool_use (команду Bash / файл Edit-Write) и шлёт
    «⏳ Claude Code ждёт подтверждения: <команда>»
— владелец из пуша видит, что дакать, ДО открытия терминала. Извлечение best-effort: транскрипт
нечитаем/формат иной → прежний текст события (не хуже старого поведения).
Зона 🟢 (конфиг агента, прод/Splinter/таблицы не трогает).
Регресс: tests/test_notify_hook_prompt.py (в гейте).
23.07.2026: дубль в локальное уведомление среды-терминала выпилен вместе с самой средой
(владелец исключил её навсегда) — единственный канал хука теперь notify.py.
"""
import sys, json
...
def build_text(data):
    ...
    msg = (data.get("message") or "ждёт твоего ответа")
    if "permission" in msg.lower():   # событие «needs your permission to use <tool>»
        cmd = _last_tool_use(data.get("transcript_path") or "")
        if cmd:
            return "⏳ Claude Code ждёт подтверждения: " + cmd
        return "⏳ Claude Code ждёт подтверждения: " + msg
...
if __name__ == "__main__":
    ...
    # force=True: это БОЕВОЕ уведомление ожидания (CC реально ждёт Филиппа) — тест-мут PRETOOL_NOPUSH его
    # НЕ глушит (регресс 05.07: широкий мут заглушил живые уведомления ожидания).
