...
ТРИ ВЕЩИ ШАГА 1:
  ...
  * отдельный адрес — канал «Лента» (`FEED_CHAT_ID`), см. notify.send_feed. В 1160 не шлём ничего:
    у инбокса РОВНО одно свойство «всё здесь ждёт меня», и заметка его размывает.

ФОРМА ЗАМЕТКИ: одна строка `🔔 <класс> · <кто> · <команда ≤120> · <исход>`. Кнопок нет, номера
карточки нет, слова «да» нет — отвечать не на что и нечем.
...
def who(data):
    tid = (os.environ.get("CC_TASK_ID") or "").strip()
    return ("задача " + tid) if tid else "Termux"
...
def send(text):
    ...
    гейт по-прежнему даёт ноль исходящих. Канал не заведён (нет FEED_CHAT_ID) → False, МОЛЧА:
