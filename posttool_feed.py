...
def _remember_permission(data, cmd, resp):
    ...
    только read-only `is_test_run` (изоляция каналов), решающих и пишущих его веток лента не
    касается по-прежнему. Сам отпечаток операции считает ГАРД в момент вопроса — лента лишь
    ...
    try:
        import repeat_ask
        from pretool_guard import is_test_run
        repeat_ask.confirm(repeat_ask.run_key(data), cmd, resp, is_test_run())
    except Exception:
        pass
