...
BRIDGE_TOOLS = [
    ...
    {
        "name": "flag_important",
        "description": "Зафиксировать ВАЖНОЕ событие (ДТП/авария, серьёзный ремонт, просрочка ТО, потеря ключа, течь, проблема с тормозами и т.п.) и закрепить напоминание в текущей теме. ВЫЗЫВАЙ ТОЛЬКО ПОСЛЕ подтверждения от Пыма/владельца ('да, закрепляй'). До подтверждения — просто спроси в чате 'это важное, закрепить? @Pleummmm подтверди'. Текст закрепа сделай коротким, двуязычным (тайский+русский), начни с ⚠️.",
        "input_schema": {
            "type": "object",
            "properties": {
                ...
                "pin_text": {"type": "string", "description": "Текст для закрепа — короткий, двуязычный TH+RU, с ⚠️ в начале"}
            },
    ...
    {
        "name": "pin_reminder",
        "description": "Закрепить важное напоминание в текущей теме (например регламент ТО, просрочка масла). Используй когда Филипп/Пым просят «закрепи», или когда видишь критичную просрочку ТО которую нужно держать на виду. Текст составь двуязычным (тайский + русский), коротким.",
        "input_schema": {
            "type": "object",
            "properties": {
                "text": {"type": "string", "description": "Текст напоминания для закрепа (двуязычный TH+RU, коротко)"}
            },
            "required": ["text"]
        }
    },
