...
# `systemctl show` под ровно те виды, что назвал адрес. К МОСТУ НЕ ХОДИМ НИКОГДА (`brain=False`):
# чтение узла мозга доходило до 543 с (замер О5), а терминал задачи — не то место, где можно
# висеть; узел поэтому остаётся непрочитанным и судья честно скажет по нему НЕИЗВЕСТНО.
#
