...
def _curator_human_render(root, items, op=None):
    ...
    {"idx","total","label","siblings":[…],"ctx":<пункт куратора дословно>}. Заголовок называет
    операцию и её номер из N, служебные строки — область «да», соседей и контекст. ПОДПИСЬ
...
def _curator_human_split(root, item, ops):
    ...
    вызывающий код откатывается на прежний путь (общая карточка) — не хуже сегодняшнего. Часть
    карточек встала, часть нет → возвращаем что встало, а НЕвставшие операции названы в отчёте
    в 328 (образец `_curator_spawn.refused`): молча не теряется ни одна.
    ...
    try:
        ...
        for i, o in enumerate(ops, 1):
            ...
            if found is not None:
                ...
                (placed if rr.get("ok") else refused).append(
                    (tid, o["label"]) if rr.get("ok") else (o["label"], "правка карточки не прошла"))
                continue
...
def _curator_human_place(root, item):
    ...
    try:
        ops = curator_ops.operations(item)
    except Exception as e:
