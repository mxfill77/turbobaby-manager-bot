...
def _card_origin(what):
    ...
    t = str(what or "")
    if ORIGIN_GUARD_TOKEN in t:
        return "guard"
    if ORIGIN_GUARD_WEAK_TOKEN in t:
        return "guard_weak"
    if ORIGIN_SELF_TOKEN in t:
        return "self"
    return "legacy"
...
def _guard_what(tid, data, verified=True):
    ...
    return f"op=other | [{hit}] {_scrub_origin(body)}\n[guard-block задача {tid}]\n{stamp}"
...
def _detect_na(text):
    """(what, kind): kind = 'marker' — модель вывела NA_MARKER (самодекларация по контракту
    преамбулы); 'phrase' — только слова о блокировке, маркера нет; None — чисто."""
    t = text or ""
...
def _curator_human_render(root, items):
    ...
    lines = [f"🧑 нужно от владельца (цель {root}) — сам куратор задач по этим пунктам НЕ ставит:"]
    for i, (t, n) in enumerate(items, 1):
        lines.append(f"{i}. {t}" + (f" (×{n})" if n > 1 else ""))
    lines.append("")
    lines.append("✅ — ПОСТАВЛЮ ЗАДАЧУ на исполнение этих пунктов (её номер придёт отдельным "
                 "рапортом в 328): пункт, просящий разрешение на операцию, будет выполнен; "
                 "пункт «просто принял к сведению» закроется без действий. ❌ — отклонить, "
                 "ничего не делаю. Новые пункты этой цели куратор дописывает в ЭТУ карточку "
                 "(актуальный список — /inbox).")
    return "\n".join(lines)[:RESULT_MAX]
...
def _convert_curator_human_approved(tid, task, what):
    ...
    FAIL-SAFE: enqueue не встал → карточка всё равно done, но result несёт пункты ДОСЛОВНО —
    «да» владельца не теряется, видно, что дожать «тз:» руками."""
    ...
    if not r.get("ok"):
        bc.complete_task(tid, "done", cap_result(
            f"🧑 сводная карточка владельцу (✅): «да» принято, но задача на исполнение НЕ встала "
            f"в очередь ({r.get('error')}). «Да» не потеряно — дожми пункты «тз:» вручную:\n{body}"))
        log.warning("curator-human: карточка %s одобрена, но задача-исполнитель не встала (%s)",
                    tid, r.get("error"))
        return
    nid = r.get("id")
    bc.complete_task(tid, "done", cap_result(
