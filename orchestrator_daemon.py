...
def process_new():
    ...
    if status == "needs_approval":
        ...
        if status == "failed" and _maybe_selfheal(tid, text, result, frm=str(task.get("from") or "")):
            return
        # FACT-верификация (R11–R15, 19.07.2026): dev-задача (from=-dev) с done и без блока FACT:
        # → метим ⚠️ unverified и форсируем куратора (bypass CURATOR_SCOPE). CURATOR=0 → тишина
        # (fail-safe). Маркер prepend'ится ДО complete_task — карточка в 328 сразу видит пометку.
        _fact_unverified = (
            status == "done"
            and str(task.get("from") or "").endswith(DEV_FROM_SUFFIX)
            and not _result_has_fact(result)
        )
        if _fact_unverified:
            result = "⚠️ unverified (нет блока FACT:)\n" + str(result or "")
            log.info("id=%s dev done без FACT: → ⚠️ unverified, форсирую куратора", tid)
        ...
        _maybe_curator_single(task.get("from"), tid, text, result, status=status,
                              force_consult=_fact_unverified)   # куратор цели (CURATOR=1)
