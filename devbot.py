...
# ПЛЕЧО 15с РУБИЛО ЖИВОЙ ОТВЕТ (15.08.2026). Мост не падал: за 74.6 суток splinter.log держит
# 3863 таймаута плеча на action=get_pending (51.8/сут) при 82 887 опросах — и у 2524 лестниц
# срублена ПЕРВАЯ попытка, а ответ был ЖИВ: по замеру основания 2037 из 2508 (81.2 %) получили
# ответ немедленным повтором. Цена: 3618 лишних повторных исполнений чтения очереди на стороне
# Apps Script и 1894 ПРОПУЩЕННЫХ тика (25.4/сут) — job ещё бежал, APScheduler держит
# max_instances=1 и следующий тик просто отменяет.
# ВЕЛИЧИНА ВЗЯТА ИЗ РАСПРЕДЕЛЕНИЯ, А НЕ С ГЛАЗА: ОДИН И ТОТ ЖЕ вызов (6 статусов, lane=all) живёт
# на ЧЕТЫРЁХ плечах, и доля срубленных опросов такова —
#   15с · этот опрос сегодня          → 3863 из 82 887 = 4.66 %
#   45с · проба наблюдателя (expectations_run.QUEUE_TIMEOUT), 5.6 сут → 1 из 753 = 0.13 %
#   60с · ЭТОТ ЖЕ опрос до 02.07.2026 → 64 из 23 332 = 0.27 %
#   90с · демон (orchestrator_daemon.py:340), 56.7 сут → ≤294 из 81 626 = ≤0.36 %
# Третья строка — самый сильный контроль: тот же клиент, тот же вызов, укорочение 60→15 подняло
# долю В 17 РАЗ, и единственное различие — длина плеча. Вся срубленная масса лежит НИЖЕ 45с,
# дальше кривая плоская — за 90с не покупается ничего измеримого. Потолок сверху ставит общий
# дедлайн карточки «Инфо» (card_deadline.BUDGET_DEFAULT, 60с): плечо обязано быть меньше него,
# иначе второй попытке не остаётся места вовсе.
# ЗАБОР ЧЕСТЕРТОНА НЕ СНЯТ, А ЗАМЕНЁН НА ЧЕСТНЫЙ. Короткое плечо было ЕДИНСТВЕННЫМ, что держало
# длину опроса: лестниц три (3 полные попытки × 6 хопов редиректа × 3 попытки эхо = 57 плеч), общего
# потолка у опроса не было — при 15с мёртвый мост держал job до 57×15 = 855с (замер связок
# таймаутов: медиана 32с, p90 707с, max 3105с). Теперь длину держит ЯВНЫЙ общий бюджет
# POLL_BUDGET, и он строже прежнего плеча: ≤60с против 855с. Откат — POLL_TIMEOUT=15 (поведение
# байт-в-байт) и/или POLL_BUDGET=0 (дедлайна нет вовсе) + рестарт splinter.
POLL_TIMEOUT = 45                       # сек на ОДНО плечо get_pending при опросе очереди
POLL_BUDGET = 60                        # сек на ВЕСЬ опрос: все три лестницы вместе (0 = без дедлайна)
...
def _poll_queue_sync(pb):
    ...
    ОБЩИЙ БЮДЖЕТ ОПРОСА (15.08.2026, см. POLL_BUDGET): единица потолка — ОДИН опрос, а не плечо.
    Потолок плеча соблюдался и раньше; ломало ЧИСЛО плеч — лестница вправе повторить его 57 раз,
    оставаясь «в бюджете» на каждом шаге. Бюджет режет все три лестницы разом и обрывает работу ДО
    сети. Исчерпан → мост отвечает ok=False (error='card_deadline') → ниже честное None, то есть
    «не прочитал», а НЕ «очередь пуста»: пустой снимок devbot прочёл бы как «задач нет»."""
    with bridge_client.card_budget(POLL_BUDGET, label="опроса очереди"):
...
def _stall_threshold_sec(it):
    ...
    не шлём). Возраст меряем по updated (heartbeat демона его освежает) — строже потолка от
    старта задачи, ложняка до потолка не даёт.

    ЗЕРКАЛО ИСПОЛНИТЕЛЯ, И ПРИЗНАК ТЕПЕРЬ ТОТ ЖЕ (правка 08.09). Полоса pc → PC_STEP_TIMEOUT
    (надзор ПК-театра, 3600). Полоса vps → ровно то, что вернёт `orchestrator_daemon._task_timeout`
    после правки 07.09 (коммит d4474de): 10 минут получает ТОЛЬКО работа, объявившая себя быстрой
    («задача:»), всё остальное — TASK_TIMEOUT_DEV. Имя отправителя порогом не распоряжается ни
    одной веткой; имена переменных окружения и дефолты по-прежнему зеркалят демона.

    ПОЧЕМУ ИМЕНИ ЗДЕСЬ БОЛЬШЕ НЕТ. До 08.09 порог выбирался по from.endswith(("-dev","-dec"))
    плюс равенство QUEUE_FROM_CURATOR. После правки исполнителя зеркало разошлось с ним: демон
    честно давал ряду ящика Штаба (from=Filipp-shtab) и ряду темы 328 (from=Filipp-328) 2700с, а
    надзор звал такой ряд зависшим через 600+120с — владелец получал ложную карточку о зависании
    РАБОТАЮЩЕГО задания. Дописать сюда ещё одно окончание имени значило бы вылечить одного
    отправителя и оставить болезнь всем следующим, поэтому сменён ПРИЗНАК, а не список имён.

    Честная оговорка о быстрой ветке: постановщик СРЕЗАЕТ командный префикс перед очередью
    (`_try_enqueue`: task_text = t[len(p):].strip()), поэтому маркер «задача:» до порога у рядов
    девбота не доезжает и быстрая ветка им недостижима — ровно как у исполнителя. Это и есть
    согласие зеркала с оригиналом: расходиться они не имеют права ни в одном ряду, и правятся
    ОДНИМ решением, а не по отдельности."""
    if _is_pc_item(it):
