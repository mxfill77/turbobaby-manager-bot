...
/**
 * Полный адрес диапазона одной строкой — для отчётов «куда именно записали».
 * Формат: "<spreadsheetId> | <sheetName> | <a1Range>".
 */
function sheetFullAddr_(spreadsheetId, sheetName, a1Range) {
  return String(spreadsheetId) + ' | ' + String(sheetName) + ' | ' + String(a1Range);
}

/**
 * Верификация записи: сбрасывает буфер (flush), затем СВЕЖИМ чтением живого
 * листа (cache-bypass) сравнивает фактические значения с ожидаемыми.
 * Сравнение нормализованное — String().trim() у обеих сторон.
 *
 * @param {Sheet}   sheet     — лист (Range берётся из него)
 * @param {number}  row       — стартовая строка (1-indexed)
 * @param {number}  col       — стартовая колонка (1-indexed)
 * @param {number}  numRows   — число строк
 * @param {number}  numCols   — число колонок
 * @param {Array<Array>} expected2D — ожидаемая 2D-матрица значений
 * @return {{ok:boolean, full_address:string, mismatches:Array<{cell:string,expected:string,got:string}>}}
 */
function verifyWrite_(sheet, row, col, numRows, numCols, expected2D) {
  SpreadsheetApp.flush();
  const range = sheet.getRange(row, col, numRows, numCols);
  const got2D = range.getValues();
  const full_address = sheetFullAddr_(
  ...
  for (let r = 0; r < numRows; r++) {
    for (let c = 0; c < numCols; c++) {
      const exp = String((expected2D[r] || [])[c]).trim();
      const act = String((got2D[r] || [])[c]).trim();
      if (exp !== act) {
        mismatches.push({
          cell: sheet.getRange(row + r, col + c).getA1Notation(),
          expected: exp,
          got: act,
        });
      }
  ...
  return { ok: mismatches.length === 0, full_address: full_address, mismatches: mismatches };
}
