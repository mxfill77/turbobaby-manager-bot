...
function setCaps(body) {
  ...
  try {
    ...
    // post-write verify: ТОЧНО записанный диапазон (шапка + строки капов)
    var writtenRange = cal.getRange(hr, col, 1 + rows.length, 3);
    var expected2D = [['Модель', 'Кап ฿/мес', 'Активен']].concat(rows);
    var full_address = sheetFullAddr_(CONFIG.SHEETS.MANAGER, CONFIG.TABS.CALENDAR,
                                      writtenRange.getA1Notation());
    var vr = verifyWrite_(cal, hr, col, 1 + rows.length, 3, expected2D);
    if (!vr.ok)
  ...
  try {
    ...
    for (var j = 0; j < vals.length; j++) {
      ...
      if (name === want) {
        var actRow = hr + 1 + j;
        var actCol = col + 2;
        var actCell = cal.getRange(actRow, actCol);
        var wantStr = on ? 'да' : 'нет';
        actCell.setValue(wantStr);

        // post-write verify: одна ячейка «Активен»
        var full_address = sheetFullAddr_(CONFIG.SHEETS.MANAGER, CONFIG.TABS.CALENDAR,
                                          actCell.getA1Notation());
        var vr = verifyWrite_(cal, actRow, actCol, 1, 1, [[wantStr]]);
        if (!vr.ok)
          return { ok: false, error: 'verify_failed', full_address: full_address, mismatches: vr.mismatches };
        ...
        return { ok: true, model: String(vals[j][0]), cap_price: (capNum && !isNaN(capNum)) ? capNum : null,
                 cap_active: on, verified: true, full_address: full_address };
      }
    }
