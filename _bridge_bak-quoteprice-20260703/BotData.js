...
const BOTDATA = {
  ...
  QUEUE_HEADERS: [
    'id', 'created', 'from', 'task_text', 'status', 'result', 'approved_by', 'updated'
  ],
 ...
 */
var QUEUE_COL = { id: 1, created: 2, from: 3, task_text: 4, status: 5, result: 6, approved_by: 7, updated: 8 };
...
/**
 * enqueue_task: новая задача в очередь. payload: { from, task_text } → { ok, id }.
...
/**
 * get_pending: задачи по статусу (деф. new), newest-first. payload: { status? } → { ok, items }.
...
function getPending_(payload) {
  ...
  var tab = getBotTab_(BOTDATA.TABS.QUEUE);
  var last = tab.getLastRow();
  if (last < 2) return { ok: true, items: [], statuses: statuses };
  var rows = tab.getRange(2, 1, last - 1, BOTDATA.QUEUE_HEADERS.length).getValues();
  var items = [];
  for (var i = rows.length - 1; i >= 0; i--) {   // newest-first
    if (statuses.indexOf(String(rows[i][QUEUE_COL.status - 1])) !== -1) items.push(queueRowObj_(rows[i]));
  }
  return { ok: true, items: items, statuses: statuses };
}

/**
 * claim_task: АТОМАРНО new→in_progress (только если ещё new, иначе already_claimed).
 * payload: { id } → { ok, task } | { ok:false, error }. LockService гарантирует, что два
