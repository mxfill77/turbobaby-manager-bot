...
ok(kind == "red" and hit == SFO, "докстринг остаётся красным намеренно (строка = возможное действие)")

# ДЕНЬГИ — ИМЕННОЕ ИСКЛЮЧЕНИЕ ИЗ ЭТОЙ АСИММЕТРИИ (02.08.2026, класс пяти пустых карточек 117…208).
# Здесь фикстура была денежной, и она же показывала цену асимметрии: у денег ложная карточка не
# просто шумит — она обходила объектный гейт и УБИВАЛА задачу. Правило денег теперь «действие, а не
# упоминание» (tests/test_money_action.py), у прочих классов асимметрия цела — строка выше.
