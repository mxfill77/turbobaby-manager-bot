...
(B-3) `pretool_guard._detail_parts` — ОБЪЕКТ карточки
      ...
      значение вычисляется». ДЕНЬГИ тогда не трогали: add_transaction/void_last ∈ _ALWAYS_CARD, их
      карточка не гасилась никогда — казалось, чинить там нечего. ПЕРЕСМОТРЕНО 02.08.2026: именно
      это исключение и пропускало владельцу пять пустых денежных карточек (117/135/140/181/208),
      оно снято, класс живёт в tests/test_money_action.py. Секция 4 сторожит границу нового правила.
...
# АСИММЕТРИЯ СНЯТА 05.08.2026 ПО РЕШЕНИЮ ВЛАДЕЛЬЦА («класс встаёт на литерал»). До этой даты
# голден стоял ровно наоборот — «докстринг остаётся красным НАМЕРЕННО (строка = возможное
# действие)» — и это было ЗАПИСАННОЕ решение 02.08, а не недосмотр: тогда правило действия завели
# только для денег, потому что у денег ложная карточка обходила объектный гейт и убивала задачу.
# Остаток был назван адресом в артефакте 02.08 §4 («распространить на имена операций ЖИВЫХ
# ТАБЛИЦ»), а живой случай полосы ПК 19:21 показал цену: красное поднялось на СТРОКЕ внутри
# python-кода, самой операции не было. Теперь правило ОДНО на все имена операций, и денежная
# фикстура ниже перестала быть исключением — она проверяет ТО ЖЕ САМОЕ.
# Регресс класса целиком: tests/test_literal_position.py.
fx_docstring = fixture("fx_doc.py", '"""Модуль про ' + SFO + ' — только описание."""\nprint(1)\n')
kind, hit, _ = classify(fx_docstring)
ok(kind == "green" and hit == "", "докстринг с именем операции ПАРКА → не красное (класс 05.08.2026)")

fx_doc_money = fixture("fx_doc_money.py", '"""Модуль про ' + AT + ' — только описание."""\nprint(1)\n')
kind, hit, _ = classify(fx_doc_money)
ok(kind == "green" and hit == "", "докстринг с ДЕНЕЖНЫМ именем → не красное (класс 02.08.2026)")
...
# Красное БЕЗ названного объекта. С 05.08.2026 это ВЫЗОВ без аргументов, а не печатаемое слово:
# имя операции в строковом литерале красного больше не даёт (класс «литерал», строка выше), и
# прежняя фикстура `print('…')` перестала бы задевать правило минимума, ради которого заведена.
fx_bare = fixture("fx_bare.py", "import bridge_client as bc\nbc." + SFO + "()\n")
kind, hit, blob = classify(fx_bare)
obj, num = PG.card_min(hit, blob)
ok(kind == "red" and hit == SFO and not obj.strip(),
   "вызов без аргументов → объекта НЕТ, карточки нет (правило минимума цело)")
...
# ПЕРЕСМОТРЕНО 02.08.2026 (класс пяти пустых денежных карточек 117/135/140/181/208): исключение
# _ALWAYS_CARD снято, правило объекта одно для ВСЕХ классов. Здесь остаётся регресс на то, что
# сняли ровно исключение и ничего больше; сам денежный класс живёт в tests/test_money_action.py.
ok(not hasattr(PG, "_ALWAYS_CARD"), "исключение _ALWAYS_CARD снято (было: деньги мимо гейта объекта)")
ok(PG.card_gate("void_" + "last", "", "") is False and PG.card_gate(AT, "", "") is False,
   "денежная карточка БЕЗ названной цели больше не рождается")
ok(PG.card_gate(AT, "кошелёк Наличка", "") is True and PG.card_gate(AT, "вызов " + AT + "()", "") is True,
   "у настоящей проводки цель есть всегда — кошелёк либо сам вызов → карточка")
