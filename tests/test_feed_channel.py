...
# подключён, поэтому сторож меняет предмет: теперь он держит ГРАНИЦУ списка — пять названных
# событий говорят, всё прочее молчит. Голдены на ДОСЛОВНЫХ командах транскриптов и ближние
# промахи (тело коммита, `stash list`, чтение маркера) — в tests/test_feed_list.py.
print("(7) список фазы 1 подключён, и граница его держится:")
for cmd, cls in (("systemctl restart splinter", "рестарт клиентского бота"),
                 ("git push origin main", "push в origin"),
...
ok(len(PF._CLASSES) == 5, "классов ровно пять (список §3 проекта, а не «сколько получилось»)")
