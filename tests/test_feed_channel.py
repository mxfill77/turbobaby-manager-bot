...
Секции:
 ...
 (7) ГРАНИЦА СПИСКА: с 04.08.2026 список фазы 1 подключён (шаг 2) — четыре класса говорят, всё
...
# ── (7) СПИСОК СОБЫТИЙ ПОДКЛЮЧЁН (шаг 2, 04.08.2026) ───────────────────────────────────────
# Шаг 1 держал здесь обратное утверждение («таблица классов пуста по построению») — это был
# сторож против того, чтобы провод молча сделал работу списка. Список утверждён владельцем и
# подключён, поэтому сторож меняет предмет: теперь он держит ГРАНИЦУ списка — ЧЕТЫРЕ названных
# события говорят, всё прочее молчит (push отозван владельцем 05.08.2026, splinter назван
# внутренним сервисом). Голдены на ДОСЛОВНЫХ командах транскриптов и ближние промахи (тело
# коммита, `stash list`, чтение маркера) — в tests/test_feed_list.py.
print("(7) список фазы 1 подключён, и граница его держится:")
for cmd, cls in (("systemctl restart splinter", "рестарт внутреннего сервиса splinter"),
                 ("pkill -f userbot_listen.py", "стоп клиентского бота"),
                 ("git reset --hard HEAD", "отброс рабочего дерева"),
                 ("git stash", "отброс рабочего дерева"),
                 ("mv tests/test_x.py /tmp/tb_scratch/", "вынос теста из гейта"),
                 ("rm -f /tmp/cc_guard_block/12.json", "стирание маркеров гарда")):
    ok(PF.classify(cmd) == cls, "%s ← %s" % (cls, cmd))
for cmd in ("venv/bin/python3 cclog.py DONE \"итог\"",
            "venv/bin/python3 gate.py",
            "git commit -q -m \"итог\"",
            "git push origin main",          # класс отозван владельцем 05.08.2026
            "systemctl restart orchestrator-daemon"):
    ok(PF.classify(cmd) is None, "вне списка — молчит: %s" % cmd)
ok(len(PF._CLASSES) == 4, "классов ровно четыре (список §3 минус отозванный push)")
ok(PF.classify(PROBE_CMD) == "проба канала", "проба канала работает по-прежнему")
