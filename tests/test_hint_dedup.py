...
Секции:
  ...
  (3) ОТРИЦАТЕЛЬНАЯ, без неё задача не закрыта: состояние ИЗМЕНИЛОСЬ → подсказка обязана
      уйти снова. Проверяется для КАЖДОГО из 15 видов на ЕГО форме состояния;
  ...
  (7) руки: все 15 видов заведены, чужих дверей `_send` на местах подсказок не осталось,
...
#  ОБЯЗАНА уйти снова, и это проверяется для КАЖДОГО из 15 видов на ЕГО форме состояния.
# ============================================================================================
CHANGED_PAIRS = {
    ...
    "J": (("damage", "Скол пластика под сиденьем"), ("damage", "Треснут нижний обтекатель"),
          "повреждение ДРУГОЕ"),
    "K": (("intent", "question", ["замена масла в редукторе"]),
          ("intent", "question", ["замена масла в редукторе", "замена колодок"]),
          "человек спросил ещё об одной работе — переспрос обязан уйти снова"),
...
class StateChangedMustSendAgain(unittest.TestCase):
    ...
    def test_lock_is_not_a_mute(self):
        """Сводно: замок, который молчит всегда, — не защита. На 15 сменах состояния 15 отправок."""
    ...
    def test_and_the_same_state_is_still_held(self):
        """Зеркало отрицательного: те же 15 видов при НЕизменном состоянии молчат все 15."""
...
class HandsInSplinter(unittest.TestCase):
    ...
    def test_all_kinds_of_the_census_are_wired(self):
        wired = set()
        ...
        self.assertEqual(sorted(wired), sorted(hint_dedup.KINDS),
                         "замок обязан стоять на ВСЕХ видах переписи, и перепись — на всех "
                         "живых вызовах: расхождение в ЛЮБУЮ сторону есть находка")
