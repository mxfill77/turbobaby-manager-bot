...
# 6) СМЕНА КОНТРАКТА (класс-фикс 4957, корень 4, 31.07.2026): пробег в шапке — ИЗ ОДНОГО ИСТОЧНИКА
#    (свой одометр «обслуживание»), а НЕ max по куче. Кейс 5849: colH 3500 (пробег ПРИ ПОКУПКЕ, к
#    текущему отношения не имеет) / свой одометр 20229 / строка «события» 20829 → шапка = 20229.
#    Почему не 20829: км из «события» — это СЛЕД РАБОТЫ, а не показание одометра; ровно так одна
#    ошибочная строка 29.07 держала 38982 поверх живых 37000/36982 двое суток (тема 79, 4957).
#    Расхождение «работа выше текущего» лечится тем, что подтверждённое число теперь попадает в сам
#    одометр (_odo_store), а рендер такой строки МОЛЧИТ — метки нет вовсе (фикс 31.07.2026: прежнее
#    «мягкое» «на текущем пробеге» было ложью, см. tests/test_card_km_label.py и карточку 4957).
class BR5849(BR):
...
m6=SENDS[-1]
print("(6) пробег = свой одометр, не км из «события» (5849):")
res.append(ok("<b>пробег 20229 км</b>" in m6 and "<b>пробег 20829 км</b>" not in m6,
              "шапка=20229 (свой одометр; colH 3500 и след работы 20829 не в счёт)"))
res.append(ok("20829 км" in m6 and "20829 км (на текущем пробеге)" not in m6 and "тормозных колодок" in m6,
              "работа 20829 ВЫШЕ текущего 20229 → строка видна, ложной метки НЕТ (20829 ≠ 20229)"))
