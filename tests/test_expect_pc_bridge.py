...
try:
    ...
    try:
        ...
        # ОТКАТ ГАСИТ ЧТЕНИЕ, КОГДА ВЫКЛЮЧЕНЫ ВСЕ ПОТРЕБИТЕЛИ ЖУРНАЛА. С 18.08.2026 их ТРОЕ:
        # О4 (следа нет вовсе), О6 (взятая задача без записи об исходе) и О7 (дети контура) —
        # одно чтение кормит всех. Поэтому ни один порог В ОДИНОЧКУ чтение НЕ гасит: иначе откат
        # соседа молча убил бы живое ожидание. Предмет проверки прежний — откат не платит мосту
        # НИ ОДНОГО вызова.
        calls[:] = []
        ER.pc_facts({}, NOW, cfg(pc=0, pc_task=0, pc_child=0))
        res.append(ok(calls == [], "(13) ОТКАТ: при выключенных всех трёх порогах "
