...
try:
    ...
    try:
        ...
        # ОТКАТ ГАСИТ ЧТЕНИЕ, КОГДА ВЫКЛЮЧЕНЫ ВСЕ ПОТРЕБИТЕЛИ ЖУРНАЛА. С 18.08.2026 их ЧЕТВЕРО:
        # О4 (следа нет вовсе), О6 (взятая задача без записи об исходе), О7 (дети контура) и О8
        # (полоса не выполняет заходов) — одно чтение кормит всех. Поэтому ни один порог В
        # ОДИНОЧКУ чтение НЕ гасит: иначе откат соседа молча убил бы живое ожидание. Предмет
        # проверки прежний — откат не платит мосту НИ ОДНОГО вызова.
        calls[:] = []
        ER.pc_facts({}, NOW, cfg(pc=0, pc_task=0, pc_child=0, lane_run=0))
        res.append(ok(calls == [], "(13) ОТКАТ: при выключенных всех четырёх порогах "
