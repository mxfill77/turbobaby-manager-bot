...
# (7) dev done БЕЗ FACT: → ⚠️ unverified + force_consult куратору
print("(7) dev done БЕЗ FACT: → ⚠️ unverified + форс куратора:")
os.environ["CURATOR_SCOPE"] = "1"  # включаем scope-фильтр
fb, tid = setup(("done", "Сводка: фикс gate.py, коммит abc1234, push done"),
                text="тз: добавь тест")
OD.process_new()
r = fb.rows[tid]
res.append(ok(r["result"].startswith("⚠️ unverified (нет блока FACT:)"),
              "result начинается с ⚠️ unverified"))
res.append(ok(r["status"] == "done", "статус остался done"))
# Куратор должен быть вызван, несмотря на CURATOR_SCOPE=1 (обычно commit не детектируется,
# т.к. «коммит» в оригинале — но scope=1 + нет commit → должен пропустить БЕЗ force.
# С force_consult=True куратор всегда вызывается.)
res.append(ok(len(consults) == 1,
              "куратор вызван (force_consult обошёл CURATOR_SCOPE)"))
# куратор получает результат уже с ⚠️
res.append(ok(consults[0][1].startswith("⚠️ unverified"),
              "куратор видит result с ⚠️ unverified"))
os.environ["CURATOR_SCOPE"] = "0"
