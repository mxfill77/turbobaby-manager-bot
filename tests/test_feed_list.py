...
Секции:
 ...
 (9) ПРОГОН НЕ КАСАЕТСЯ БОЕВЫХ ФАЙЛОВ СОСТОЯНИЯ — прямой ответ на инцидент 04.08.2026, когда
     ...
     каталоге с уникальным суффиксом, боевые каталоги до и после прогона побайтно те же.
...
# ── СНИМОК БОЕВЫХ КАТАЛОГОВ ДО ПРОГОНА (секция 9 сверит после) ─────────────────────────────
...
def snapshot(paths):
    ...
    for p in paths:
        try:
            out[p] = sorted((n, os.path.getmtime(os.path.join(p, n))) for n in os.listdir(p))
        except OSError:
            out[p] = None            # каталога нет — тоже факт, и он обязан не измениться
    return out


BEFORE = snapshot(LIVE_STATE)
...
ok(os.path.isdir(SEEN) or True, "каталог прогона убирается в конце (см. rmtree ниже)")
