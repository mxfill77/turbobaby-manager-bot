...
os.environ["ASK_DEDUP"] = "0"         # ЛЕГАСИ-ИЗОЛЯЦИЯ (22.08.2026): предмет сьюта — ПРЕЖНЕЕ
# поведение двери, а дедуп между дверьми глушит ВТОРОЙ вопрос об ОДНОМ объекте — сьют же гоняет
# один и тот же пункт помногу раз. Без «0» он вдобавок писал бы БОЕВОЙ реестр (фикстура в живом
# состоянии). Предмет сьюта не изменён: при «0» сьют зелёный, при «1» падают РОВНО повторные
# постановки одного пункта — штатный вклад дедупа, а не дефект предмета.
