"""АДРЕС РЕЗУЛЬТАТА ШАГА — поле очереди заведено, и оно ИНЕРТНО (16.08.2026).
...
_SAMPLES = [
    ("commit", "a3cf244"),
    ("file", "/root/turbobaby-manager-bot/result_ref.py"),
...
ok(RR.parse("[адрес результата: выдумка что-то]") is None,
   "неизвестный вид адресом не считается (и разбор не падает)")
...
ok(_twice.count("[адрес результата:") == 1 and RR.parse(_twice)["kind"] == "file",
...
TEXTS = [
    ...
    "",
    "я" * SHEET_CAP,
    "хвост похож на адрес, но им не является: [адрес результата: выдумка x]",
]
...
try:
    for label, out, rc in [("зелёный шаг", "сводка: сделано", 0),
        ...
        ok(plain[0] == with_ref[0] == with_file[0],
           f"{label}: статус тот же, что без адреса ({plain[0]})")
        ok(plain[1] == with_ref[1] == with_file[1],
