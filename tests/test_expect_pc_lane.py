...
try:
    ...
    try:
        ...
        # Ветки выключены → журнал не читается вовсе. ЧИТАТЕЛЕЙ У НЕГО СТАЛО ЧЕТВЕРО (18.08.2026:
        # О7 о детях контура и О8 о чередах внешних отказов читают ТОТ ЖЕ текст ТЕМ ЖЕ вызовом),
        # поэтому молчание мосту покупает только их ОБЩИЙ откат — предмет проверки прежний,
        # знаменатель честный.
        calls[:] = []
        ER.pc_facts({}, TOOK_43, cfg(pc=0.0, pc_task=0.0, pc_child=0.0, lane_run=0.0))
        res.append(ok(not calls, "(15) все четыре порога 0 → мост не зовётся"))
