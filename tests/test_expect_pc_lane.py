...
try:
    ...
    try:
        ...
        # Обе ветки выключены → журнал не читается вовсе.
        calls[:] = []
        ER.pc_facts({}, TOOK_43, cfg(pc=0.0, pc_task=0.0))
        res.append(ok(not calls, "(15) EXPECT_PC_MIN=0 И EXPECT_PC_TASK_MIN=0 → мост не зовётся"))
