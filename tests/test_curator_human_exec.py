...
Проверки:
 ...
 (8) FAIL-SAFE: enqueue не встал → карточка закрыта ЧЕСТНО (не зависает), пункты в result
     ДОСЛОВНО («да» владельца не теряется), петли нет. ИСХОД СМЕНЁН 04.08.2026 (вторая ревизия
     класса): было done — а done в 328 читается РОВНО как успех, и владелец видел зелёный
     рапорт при отсутствующей задаче, то есть тот же класс с другой стороны. Теперь failed;
     сама постановка идёт надёжно (verify → повтор), см. tests/test_curator_button_work.py;
...
res.append(ok(fb.rows[cid]["status"] == "failed",
              "карточка закрыта FAILED (не зависает и не выглядит успехом — правка 04.08.2026)"))
