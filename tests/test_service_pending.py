...
def test_a1_write_idempotent_content_msgid():
    # A1: msg_id по контенту info:plate:КЛЮЧ:км → повторный прогон (др.формулировка) = тот же ключ.
    # СМЕНА КОНТРАКТА (класс-фикс 4957, корень 3, 31.07.2026): ключ = стем + УТОЧНИТЕЛЬ МЕСТА.
    # Прежний голый стем схлопывал «передние колодки» и «задние колодки» в один ключ — вторая
    # работа приходила duplicate=True и исчезала молча (splinter.log 29.07 09:32:18). Поэтому
    # «переднего подшипника» теперь даёт «подшипник/перед», а не «подшипник».
    reset()
    b = FakeBridge()
    ...
    assert mids1 == ["info:4255:колодки:24302", "info:4255:подшипник/перед:24302"], mids1
    n = len(b.events)
