...
s2, _ = deliver("baf5d30", bridge=fb_a)
led_a = ledger()
print("   прогон 1: коммит b5478ce → карточки владельцу %s" % s1)
print("   прогон 2: коммит baf5d30 → карточки владельцу %s" % s2)
...
res.append(ok(led_a["skipped"] == 0, "счётчик дедупа НЕ рос — глушить было нечего: %s"
              % led_a["skipped"]))
res.append(ok(sorted(led_a["asks"]) == ["service:splinter|b5478ce", "service:splinter|baf5d30"],
              "в реестре ровно два разных объекта"))
