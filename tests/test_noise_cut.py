...
def sec5():
    ...
    try:
        ...
        # Развод пункта по операциям, где часть карточек НЕ встала: заметка «операции БЕЗ
        # карточки» живёт под ⚠️ — без неё развод выглядел бы полностью удавшимся.
        split_ok = _card_body("задача", 264, "human", human="разреши деплой и рестарт",
                              hum=("341, 342", "split"))
        line = devbot._curator_line(split_ok)
        ok(bool(line) and "341, 342" in line, "развод без сбоев ужимается в строку: %s" % line)
        split_bad = _card_body("задача", 264, "human", human="разреши деплой и рестарт",
                               hum=("341", "split", "операции БЕЗ карточки (дожать «тз:» руками): "
                                    "рестарт splinter — enqueue не прошёл"))
        ok(devbot._curator_line(split_bad) is None,
           "часть операций без карточки (⚠️) → карточка идёт целиком")
