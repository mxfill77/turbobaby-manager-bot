...
неизвестно»; маркеры очереди «[ревизор …]» — не тики; рядом со штампом — ВОЗРАСТ и пометка
«⚠️ тик старый» (шаг 2 цели 531, 14.08.2026; голдены числа — `tests/test_revizor_age.py`, здесь
проверяется только сама строка доски). Всё пусто → «🟢 ТИХО»; Bridge-молчит ≠ ТИХО.
...
res.append(ok("👁 надзор: ревизор 2026-08-13 13:26 (" in out and "— окон 1" in out,
              "живая строка «NOTE <штамп> UTC: Orchestrator: ревизор: 1 окон, чисто» разобрана"))
# возраст считается от ЖИВЫХ часов (у `_g_pulse` времени не пришпилить), а фикстура — из прошлого:
# поэтому проверяем не число, а то, что возраст назван и назван СТАРЫМ (голден числа — в
# tests/test_revizor_age.py, где «сейчас» приносят снаружи)
res.append(ok("назад" in out.split("👁 надзор:")[-1] and "⚠️ тик старый" in out,
              "возраст тика назван рядом со штампом, вчерашний тик помечен старым (шаг 2 цели 531)"))
...
res.append(ok(DB._revisor_line(lambda: _COWORK_LIVE, hours=0)
...
res.append(ok(DB._revisor_line(lambda: legacy, hours=0) == "👁 надзор: ревизор — окон 2",
              "легаси-NOTE без штампа: окна есть, время не выдумано"))
res.append(ok(DB._revisor_line(lambda: legacy)
              == "👁 надзор: ревизор (возраст неизвестен: время тика не названо) — окон 2",
              "у легаси без штампа возраст ИЗМЕРИТЬ НЕЧЕМ — и это сказано, а не замолчано"))
