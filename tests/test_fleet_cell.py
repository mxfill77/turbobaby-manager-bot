...
# Кто СОЗНАТЕЛЬНО просит разметку. Пусто — значит сегодня её не просит никто, и любой новый
# потребитель обязан появиться здесь ЯВНО, вместе с ответом «что он будет делать с unreadable».
FLEET_CELLS_CONSUMERS = frozenset()
