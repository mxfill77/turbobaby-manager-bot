...
# Остаток у 135 ТОТ ЖЕ, что у 117, и назван так же честно: имя операции ЖИВОЙ ТАБЛИЦЫ в данных
# по-прежнему красит команду (ветка `_body_has`, не денежная). Владельца не будит — объекта нет.
ok(b == "журнал" and hit == CB,
   "135: остаток тот же, что у 117 — имя операции CRM из данных (hit=%s, %s)" % (hit, b))
