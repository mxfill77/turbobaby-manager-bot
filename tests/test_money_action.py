...
ЧТО ПРОВЕРЯЕТСЯ ЗДЕСЬ:
  (1) дословные тела 208, 117 и 135 карточки НЕ рождают (135 — харнесс собственного регресса той
      задачи, 31.07 16:20:14 UTC: имена операций лежат ДАННЫМИ в списке случаев, включая текст
      настоящей проводки в подписи D2; голден сверен с транскриптом по длине и sha1);
...
(`a739ee9` + `PRETOOL_BLOCK_DIR` в фикстуре). Обе стороны 181 проверяются в секции (2):
дословная BASH-команда прогона (02.08 07:04:50 UTC) карточки НЕ рождает — она зелёная; тело
фикстуры MONEY_CALL карточку рождает и ДОЛЖНО, с названными кошельком и суммой.
...
# ОСТАТОК ЗАКРЫТ 05.08.2026 («класс встаёт на литерал»). Здесь стояло обратное: тот же список слов
# содержит имена операций ЖИВЫХ ТАБЛИЦ, и они ПРОДОЛЖАЛИ красить команду — владельца это не будило
# (объекта нет, карточка гаснет), но команда без «да» не шла, а в headless «да» некому дать.
# Теперь правило действия одно на ВСЕ имена операций, и дословная 117 уходит в defer ЦЕЛИКОМ —
# включая написание confirmed=true, которое лежит в том же списке ДАННЫМИ (канала у тела нет:
# импорт ровно один, re). Регресс класса: tests/test_literal_position.py.
ok(b == "defer", "117 ДОСЛОВНО: defer целиком, остаток закрыт (%s, hit=%s)" % (b, hit or "—"))
ok(hit != CB, "117: имя операции CRM из списка слов класс больше не назначает (hit=%s)" % (hit or "—"))

# ДОСЛОВНАЯ команда 135 (транскрипт 2026-07-31T16:20:14 UTC, карточка 16:20:16) — ХАРНЕСС
# СОБСТВЕННОГО РЕГРЕССА той задачи: список случаев `cases = [(подпись, команда), …]`, где имена
# операций лежат ДАННЫМИ внутри строковых литералов, а вызовы тела — только G.classify/card_min/
# card_gate/print. Случай D2 несёт текст НАСТОЯЩЕЙ проводки (`BridgeClient().add_transaction(...)`)
# — но как строку-данные, не как вызов: ровно та разница, которую теперь видит разбор.
# Байт-в-байт со снятой командой: длина 1163, sha1 b9da5ebea04d (генератор
# `_scratch_cur212_0802/gen_golden.py` сверяет склейку с транскриптом).
CMD_135 = (
    "PRETOOL_NOPUSH=1 venv/bin/python3 -c '\n"
    'import sys\n'
    'sys.path.insert(0, "/root/turbobaby-manager-bot")\n'
    'import pretool_guard as G\n'
    'TASK117 = open("/dev/stdin").read()\n'
    '\' </dev/null; echo "---"; PRETOOL_NOPUSH=1 venv/bin/python3 -c \'\n'
    'import sys\n'
    'sys.path.insert(0, "/root/turbobaby-manager-bot")\n'
    'import pretool_guard as G\n'
    'cases = [\n'
    ' ("D1 читающий скрипт со СПИСКОМ слов (задача 117)", "venv/bin/python3 -c \\x27\\nsrc = '
    'open(\\"/root/turbobaby-manager-bot/splinter.py\\").read()\\nwrites = [\\"' + SF + '\\", \\"'
    + AT + '\\", \\"' + VL + '\\", \\"' + CB + '\\"]\\nfor w in writes:\\n    print(w, '
    'src.count(w))\\n\\x27"),\n'
    ' ("D2 РЕАЛЬНАЯ проводка с аргументами", "venv/bin/python3 -c \\x27import bridge_client as b; '
    'b.BridgeClient().' + AT + '(group=\\"MoneyPhuket\\", amount=500)\\x27"),\n'
    ' ("D3 РЕАЛЬНАЯ отмена без аргументов", "venv/bin/python3 -c \\x27import bridge_client as b; '
    'b.BridgeClient().' + VL + '()\\x27"),\n'
    ']\n'
    'for lab, c in cases:\n'
    '    kind, hit, blob = G.classify(c, "/root/turbobaby-manager-bot")\n'
    '    o, n = G.card_min(hit, blob)\n'
    '    gate = G.card_gate(hit, o, n)\n'
    '    print("%-46s kind=%-6s hit=%-16s объект=%-22r число=%-14r КАРТОЧКА=%s" % (lab, kind, hit, '
    'o, n, gate))\n'
    "'"
)
ok(len(CMD_135) == 1163 and hashlib.sha1(CMD_135.encode("utf-8")).hexdigest()[:12] == "b9da5ebea04d",
   "135: голден склеен ДОСЛОВНО (len=%d, sha1=%s)"
   % (len(CMD_135), hashlib.sha1(CMD_135.encode("utf-8")).hexdigest()[:12]))
b, hit, obj, num = bucket(CMD_135)
ok(hit != AT, "135 ДОСЛОВНО: денежный класс к харнессу больше не относится (hit=%s)" % hit)
ok(b != "КАРТОЧКА", "135 ДОСЛОВНО: карточки владельцу НЕТ — %s (объект=%r)" % (b, obj))
# Остаток у 135 был ТОТ ЖЕ, что у 117, и закрыт тем же правилом 05.08.2026: имя операции ЖИВОЙ
# ТАБЛИЦЫ, лежащее в данных, команду больше не красит (ветка `_body_has` — одна на все имена).
ok(b == "defer", "135 ДОСЛОВНО: defer целиком, остаток закрыт (%s, hit=%s)" % (b, hit or "—"))
ok(hit != CB, "135: имя операции CRM из данных класс больше не назначает (hit=%s)" % (hit or "—"))
...
# ДОСЛОВНАЯ команда 181 (транскрипт 2026-08-02T07:04:50 UTC) — сам прогон подмножества тестов,
# внутри которого лежит голден MONEY_CALL. Для гарда она ЗЕЛЁНАЯ: карточку рождает не запуск
# тестов, а тело фикстуры, которое тест пишет на диск. Голден держит эту границу.
CMD_181_OUTER = (
    "venv/bin/python3 _scratch/roota_20260802/_run_subset.py test_pretool_hardblock "
    "test_pretool_probe_dedup test_guard_card_min test_guard_escalation test_probe_isolation "
    "test_delete_scope test_pretool_commit_msg test_pretool_infoflags test_no_push_leak "
    "test_guard_cli_args"
)
ok(len(CMD_181_OUTER) == 269, "181: голден прогона склеен ДОСЛОВНО (len=%d)" % len(CMD_181_OUTER))
b, hit, _o, _n = bucket(CMD_181_OUTER)
ok(b == "defer", "181 ДОСЛОВНО (прогон тестов): карточки нет — %s (hit=%s)" % (b, hit or "—"))

# А ВОТ ТЕЛО ФИКСТУРЫ, из которого карточка 181 и ушла владельцу, — MONEY_CALL из
# tests/test_pretool_probe_dedup.py. ЧЕСТНО: карточка тут РОЖДАЕТСЯ и рождаться ДОЛЖНА — это
# настоящий разобранный вызов с кошельком и суммой, а не совпадение слова (иначе мы ослабили бы
# денежный класс до слепоты). Владельцу она не должна была ДОЕХАТЬ — за это отвечает изоляция
# проб (`a739ee9` + `PRETOOL_BLOCK_DIR` в фикстуре), и она здесь не трогается.
...
# 05.08.2026: парк перестал быть «соседним классом» здесь — правило действия распространено на ВСЕ
# имена операций, и голое упоминание в строке ведёт себя ТАК ЖЕ, как денежное: defer. Прежде тут
# проверялось обратное («красное без карточки, как было») — это и был честно названный остаток §4.
b, hit, obj, _n = bucket(fixture("fx_oil_bare.py", "print('" + SFO + "')\n"))
ok(b == "defer", "парк: голое упоминание в строке — defer, как у денег (%s, hit=%s)" % (b, hit or "—"))
# Правило минимума проверяем тем, что красное действительно даёт: вызовом без названного объекта.
b, hit, obj, _n = bucket(fixture("fx_oil_call_bare.py", "import bridge_client as bc\nbc." + SFO + "()\n"))
ok(hit == SFO and b == "журнал",
   "парк: вызов без объекта — красное без карточки (%s, hit=%s)" % (b, hit))
