...
# HQ-форум TurboControl. Темы внутри него разные и адресуются ПАРОЙ (чат, тема):
# инбокс «ждут владельца» = INBOX_TOPIC_ID, лента «сказать, не спрашивая» = PC_DEV_TOPIC_ID.
...
def send_inbox(text, answerable, buttons=None):
    ...
    кураторские сводки с решением. Информационное («сделано», «сводка», «статус») идёт в ленту
    829, потому что у инбокса ровно одно свойство — «всё здесь ждёт меня», и одно сообщение без
    вопроса это свойство размывает на все остальные.

    ПОЧЕМУ ДВЕРЬ, А НЕ ПРАВИЛО В ГОЛОВЕ. Замер 05.08.2026 показал, что сегодня в 1160 не пишет
    НИЧЕГО информационного (три отправителя, все с ответом — разбор в артефакте) — то есть
    убирать было нечего, а вот удержать нечем: любой следующий «просто скажу владельцу» лёг бы
    туда же, и правило пришлось бы вводить заново после инцидента. Теперь адрес выбирает не
    автор сообщения, а признак `answerable`, и «не требует ответа» физически не имеет пути в
    1160 — даже фолбэком (см. ниже).

    answerable=False → ЛЕНТА (send_feed) и НИКУДА больше. Фолбэка в инбокс/личку тут нет
    намеренно: адреса ленты нет → сообщение не уходит вовсе. «Уходит куда-нибудь» — это ровно
    тот отказ, ради которого дверь и заводится.
    answerable=True → прежний маршрут: тема-инбокс HQ (INBOX_TOPIC_ID, прод 1160), ЛИЧКА =
    ФОЛБЭК (инбокс выключен ИЛИ форум недоступен). → (message_id, chat_id) | None.
    """
    if not answerable:
        send_feed(text)
        return None
...
FEED_TOPIC_ENV = "PC_DEV_TOPIC_ID"   # тема «ПК-дев» HQ-форума = адрес ленты (решение владельца)


def _feed_dest():
    """(chat_id, thread_id) ленты — заметки, на которые НЕ отвечают. None = адреса нет → МОЛЧИМ.

    РЕШЕНИЕ ВЛАДЕЛЬЦА 03.08.2026: лента живёт в теме «ПК-дев» — тот же чат, что у инбокса, но
    ДРУГАЯ тема. Отдельного канала не заводим (свободных тем нет, новую при заморозке клиентского
    контура открывать опасно), новых ключей тоже: чат — константа выше, тема — уже живущий
    `PC_DEV_TOPIC_ID`. Основание, что тема инертна: Splinter в ней молчит РАНО, до мозга и учёта
    (`splinter.is_ignored_thread` знает её по этому же ключу); devbot принимает только владельца и
    только по префиксам; заметку шлёт ТОТ ЖЕ бот, а свои сообщения Telegram боту назад не отдаёт →
    петли нет по устройству, а не по аккуратности текста.

    АДРЕС — ИМЕННО ПАРА, как у _inbox_dest, и это не украшение: `chat_id` без `message_thread_id`
    уводит сообщение в General форума, то есть в чужую тему на глазах у всех. Поэтому прежний ключ
    отдельного чата (`FEED_CHAT_ID`) снят, а не оставлен запасным путём: он был единственной
    веткой БЕЗ темы — ровно той, что промахивается в General.

    Темы нет/0/мусор → None: заметка не уходит НИКУДА, а не «уходит куда-нибудь» (фолбэка у ленты
    нет — см. send_feed). .env грузим сами (как в _inbox_dest): posttool_feed зовёт send_feed из
    hook-процесса, где .env ещё не загружен; load_dotenv существующие env НЕ перекрывает."""
    try:
        from dotenv import load_dotenv
        load_dotenv(os.path.join(os.path.dirname(os.path.abspath(__file__)), ".env"))
        t = int((os.getenv(FEED_TOPIC_ENV) or "0").strip() or 0)
    except Exception:
        return None
    return (HQ_CHAT_ID, t) if t else None
...
def send_feed(text) -> bool:
    ...
    АДРЕС ОТДЕЛЬНЫЙ И ФОЛБЭКА НЕТ НАМЕРЕННО. Тема-инбокс держит РОВНО одно свойство — «всё здесь
    ждёт меня»; заметка, на которую не отвечают, это свойство размывает, поэтому лента живёт в
    СВОЕЙ теме того же форума (_feed_dest). Личка — канал алармов (notify_hook «Termux ждёт
    подтверждения», health): разбавить аларм рутиной = обучить его игнорировать. Поэтому адреса
    нет → сообщение НЕ уходит никуда, а не «уходит куда-нибудь».
    ...
    if not token:
        return False
    ok, _mid = _send_message(token, text, chat_id=dest[0], thread_id=dest[1])
    return ok
