...
# --------------------------------------------------------------------------------------------
#  ИНВАРИАНТ 12в-бис: PARK_VERDICT_PURE
#  Исход по регистру ТО (park_verdict.py) отвечает на один вопрос: «в норме, просрочено или мы
#  НЕ ЗНАЕМ?» — и судит РОВНО то, что ему принесли. Появись у него руки, он смог бы сходить за
#  клеткой или за интервалом сам, и тогда «в норме» зависело бы от того, КАК спросили, а не от
#  того, что в клетке; хуже того, руками можно было бы ДОБРАТЬ недостающее и назвать это нормой —
#  ровно тот фантом, из-за которого перепись 08.08 насчитала 12 ложных просрочек из 35.
#  Импортов РОВНО ДВА, и оба — готовый вокабуляр: `fleet_cell` (какие поля суть регистры) и
#  `scan_result` (какими бывают исходы чтения клетки). Своего перечисления о тех же смыслах
#  модуль не заводит: два словаря об одном разошлись бы молча. Разбор — тот же ast, что у
#  дежурного: имя в комментарии, строке и докстринге кодом не является.
#  FAIL-CLOSED: файла нет / не парсится → ФЛАГ: нечитаемое решение доверия не имеет.
# --------------------------------------------------------------------------------------------
_PARK_VERDICT_PATH = None       # подменяется САМОТЕСТОМ; None → боевой park_verdict.py в репо
_PARK_VERDICT_ALLOWED_IMPORTS = frozenset(("fleet_cell", "scan_result"))


@register("PARK_VERDICT_PURE")
def check_park_verdict_pure(world, run):
    path = _PARK_VERDICT_PATH or os.path.join(REPO, "park_verdict.py")
    try:
        with open(path, encoding="utf-8") as f:
            src = f.read()
    except OSError as e:
        run.flag("park_verdict.py", f"решение об исходе регистра не читается ({e}) — "
                                    f"чистота не доказана")
        return
    try:
        findings = _duty_ast_findings(src, allowed=_PARK_VERDICT_ALLOWED_IMPORTS)
    except SyntaxError as e:
        run.flag("park_verdict.py", f"решение об исходе регистра не разбирается ({e}) — "
                                    f"чистота не доказана")
        return
    for where, why in findings:
        run.flag(f"park_verdict.py:{where}", why)
...
#  появись у модуля сеть — он смог бы спросить очередь САМ, и «сменилось» стало бы зависеть от
#  того, КОГДА спросили, а не от принесённых фактов; появись время — «снято в …» перестало бы
#  быть временем СНЯТИЯ и стало бы временем РЕНДЕРА, то есть слепок начал бы врать свежестью
#  ровно тем способом, против которого написан; появись файл — он дописал бы документ мимо
#  единственной двери записи. Импортов РОВНО ДВА: `datetime` — только ради меток UTC, и
#  `owner_refusal` (22.08.2026) — ВОКАБУЛЯР исходов закрытой задачи, взятый ГОТОВЫМ. Второй
#  разрешён по той же причине, по какой у `service_receipt` разрешён `write_fact`: у самого
#  `owner_refusal` импортов НОЛЬ (страж OWNER_REFUSAL_PURE ниже), рук он не приносит, а второе
#  определение «чей это исход» рядом со слепком было бы ровно тем, против чего написаны оба
#  модуля — двумя ответами на один вопрос.
#  FAIL-CLOSED: файла нет / не парсится → ФЛАГ: недоказанная чистота доверия не имеет.
# --------------------------------------------------------------------------------------------
_QUEUE_STATE_PATH = None        # подменяется САМОТЕСТОМ; None → боевой queue_state.py в репо


@register("QUEUE_STATE_PURE")
def check_queue_state_pure(world, run):
    path = _QUEUE_STATE_PATH or os.path.join(REPO, "queue_state.py")
    try:
        with open(path, encoding="utf-8") as f:
            src = f.read()
    except OSError as e:
        run.flag("queue_state.py", f"решение слепка очереди не читается ({e}) — чистота не доказана")
        return
    try:
        findings = _duty_ast_findings(src, allowed=frozenset(("datetime", "owner_refusal")))
    except SyntaxError as e:
        run.flag("queue_state.py",
                 f"решение слепка очереди не разбирается ({e}) — чистота не доказана")
        return
    for where, why in findings:
        run.flag(f"queue_state.py:{where}", why)


# --------------------------------------------------------------------------------------------
#  ИНВАРИАНТ 12ф: OWNER_REFUSAL_PURE
#  Признак «это решение владельца, а не сбой» (owner_refusal.py) отвечает на ОДИН вопрос: чей
#  исход у закрытой строки. Импортов РОВНО НОЛЬ, и это и есть замок: появись у него сеть — он
#  спросил бы очередь САМ, и ответ стал бы зависеть от того, КОГДА спросили; появись запись —
#  рядом с единственным местом, где исход называется, завёлся бы второй путь, и «отклонена»
#  можно было бы ПОСТАВИТЬ, а не только ПРОЧИТАТЬ; появись время — «переотправке не подлежит»
#  начало бы стареть само собой. Судить он умеет РОВНО принесённый текст.
#  FAIL-CLOSED: файла нет / не парсится → ФЛАГ: недоказанная чистота доверия не имеет.
# --------------------------------------------------------------------------------------------
_OWNER_REFUSAL_PATH = None      # подменяется САМОТЕСТОМ; None → боевой owner_refusal.py в репо


@register("OWNER_REFUSAL_PURE")
def check_owner_refusal_pure(world, run):
    path = _OWNER_REFUSAL_PATH or os.path.join(REPO, "owner_refusal.py")
    try:
        with open(path, encoding="utf-8") as f:
            src = f.read()
    except OSError as e:
        run.flag("owner_refusal.py",
                 f"решение об отказе владельца не читается ({e}) — чистота не доказана")
        return
    try:
        findings = _duty_ast_findings(src, allowed=frozenset())
    except SyntaxError as e:
        run.flag("owner_refusal.py",
                 f"решение об отказе владельца не разбирается ({e}) — чистота не доказана")
        return
    for where, why in findings:
        run.flag(f"owner_refusal.py:{where}", why)
...
# --------------------------------------------------------------------------------------------
#  ИНВАРИАНТ 12ч: ODO_FRESH_PURE
#  Срок годности подтверждённого пробега (odo_fresh.py, 23.08.2026) решает, спрашивать ли число,
#  которое бот уже знает, — и стоит ПЕРЕД дверью живой таблицы Лист1: снятый зря вопрос уводит в
#  запись протухший одометр, а лишний стоит одного оборота. Импорт РОВНО ОДИН — `datetime`:
#  разобрать время строки листа и вычесть его из ПРИНЕСЁННОГО «сейчас». Будь у решения сеть, оно
#  спросило бы одометр САМО, и «свежесть» снова зависела бы от того, КАК спросили, а не от строки,
#  по которой идёт запись; будь у него часы — голдены поехали бы за живым временем (тот же класс,
#  что «мок, переставший задевать ветку»); будь у него запись — рядом с вопросом завёлся бы второй
#  путь в живые таблицы мимо «да» доверенного.
#  FAIL-CLOSED: файла нет / не парсится → ФЛАГ: недоказанная чистота доверия не имеет.
# --------------------------------------------------------------------------------------------
_ODO_FRESH_PATH = None          # подменяется САМОТЕСТОМ; None → боевой odo_fresh.py в репо


@register("ODO_FRESH_PURE")
def check_odo_fresh_pure(world, run):
    path = _ODO_FRESH_PATH or os.path.join(REPO, "odo_fresh.py")
    try:
        with open(path, encoding="utf-8") as f:
            src = f.read()
    except OSError as e:
        run.flag("odo_fresh.py", f"решение свежести пробега не читается ({e}) — чистота не доказана")
        return
    try:
        findings = _duty_ast_findings(src, allowed=frozenset(("datetime",)))
    except SyntaxError as e:
        run.flag("odo_fresh.py",
                 f"решение свежести пробега не разбирается ({e}) — чистота не доказана")
        return
    for where, why in findings:
        run.flag(f"odo_fresh.py:{where}", why)


# --------------------------------------------------------------------------------------------
#  ИНВАРИАНТ 12ш: ODO_LOWER_PURE
#  Понижение пробега с письменным пояснением (odo_lower.py, 23.08.2026) решает, пройдёт ли число
#  ВНИЗ и что об этом сохранится, — то есть стоит ПЕРЕД дверью живой таблицы Лист1. Импорт РОВНО
#  ОДИН — `re`: разобрать текст пояснения и отличить слова человека от односложного согласия.
#  Будь у решения сеть, оно спросило бы записанное число САМО, и «расхождение» снова зависело бы
#  от того, КАК спросили, а не от строки, по которой идёт запись; будь у него запись — рядом с
#  гейтом пояснения завёлся бы второй путь в живую таблицу, мимо самого пояснения, ради которого
#  правило владельца и заведено.
#  FAIL-CLOSED: файла нет / не парсится → ФЛАГ: недоказанная чистота доверия не имеет.
# --------------------------------------------------------------------------------------------
_ODO_LOWER_PATH = None          # подменяется САМОТЕСТОМ; None → боевой odo_lower.py в репо


@register("ODO_LOWER_PURE")
def check_odo_lower_pure(world, run):
    path = _ODO_LOWER_PATH or os.path.join(REPO, "odo_lower.py")
    try:
        with open(path, encoding="utf-8") as f:
            src = f.read()
    except OSError as e:
        run.flag("odo_lower.py", f"решение понижения пробега не читается ({e}) — чистота не доказана")
        return
    try:
        findings = _duty_ast_findings(src, allowed=frozenset(("re",)))
    except SyntaxError as e:
        run.flag("odo_lower.py",
                 f"решение понижения пробега не разбирается ({e}) — чистота не доказана")
        return
    for where, why in findings:
        run.flag(f"odo_lower.py:{where}", why)


# --------------------------------------------------------------------------------------------
#  ИНВАРИАНТ 12ъ: WORK_INTENT_PURE
#  Намерение приёма работ (work_intent.py, 30.08.2026) решает, РОДИТСЯ ли заявка из сообщения
#  человека, — то есть стоит ПЕРЕД дверью `service_pending_upsert`. Импорт РОВНО ОДИН — `re`:
#  разрезать сообщение на клаузы и найти в клаузе вопросительный знак или частицу. Будь у
#  решения мост, оно спросило бы очередь САМО, и «вопрос это или отписка» зависело бы от того,
#  что уже записано, а не от того, что человек сказал; будь у него запись — рядом с гейтом
#  намерения завёлся бы второй путь в заявку, мимо самого намерения, ради которого он заведён.
#  FAIL-CLOSED: файла нет / не парсится → ФЛАГ: недоказанная чистота доверия не имеет.
# --------------------------------------------------------------------------------------------
_WORK_INTENT_PATH = None        # подменяется САМОТЕСТОМ; None → боевой work_intent.py в репо


@register("WORK_INTENT_PURE")
def check_work_intent_pure(world, run):
    path = _WORK_INTENT_PATH or os.path.join(REPO, "work_intent.py")
    try:
        with open(path, encoding="utf-8") as f:
            src = f.read()
    except OSError as e:
        run.flag("work_intent.py",
                 f"решение намерения работ не читается ({e}) — чистота не доказана")
        return
    try:
        findings = _duty_ast_findings(src, allowed=frozenset(("re",)))
    except SyntaxError as e:
        run.flag("work_intent.py",
                 f"решение намерения работ не разбирается ({e}) — чистота не доказана")
        return
    for where, why in findings:
        run.flag(f"work_intent.py:{where}", why)


# --------------------------------------------------------------------------------------------
#  ИНВАРИАНТ 12щ: REPLY_FLOOR_PURE
#  Пол ответа (reply_floor.py, 23.08.2026) решает, ЧТО сказать человеку, когда сказать нечего и
#  когда отказано. Импорт РОВНО ОДИН — `write_fact`: вопрос «мост вынес отказ САМ или это
#  молчание транспорта» он уже решает (`SETTLED_ERRORS`), и второго списка о том же смысле здесь
#  не заводится — разойдясь, они дали бы человеку два разных ответа об одном отказе. Будь у
#  решения отправка, «бот не молчит» зависело бы от того, дошло ли сообщение, а не от того, есть
#  ли что сказать; будь у него сеть — он спрашивал бы мир САМ, и слова отказа поехали бы за
#  состоянием мира вместо состояния ответа.
#  FAIL-CLOSED: файла нет / не парсится → ФЛАГ: недоказанная чистота доверия не имеет.
# --------------------------------------------------------------------------------------------
_REPLY_FLOOR_PATH = None        # подменяется САМОТЕСТОМ; None → боевой reply_floor.py в репо


#  --------------------------------------------------------------------------------------------
#  ИНВАРИАНТ 12ы: BATCH_ODO_PURE
#  Партия работ на ДРУГОМ пробеге (batch_odo.py, 25.08.2026) решает, ложится ли партия и на какие
#  пробег с датой, — то есть стоит ПЕРЕД дверью истории байка. Импортов РОВНО ТРИ и каждый назван:
#  `re` (разобрать число и дату из свободного текста), `datetime` (есть ли такая дата в календаре;
#  СВОИХ ЧАСОВ у решения нет — «сегодня» приносят руки параметром), `odo_lower` (готовая дорога
#  понижения: её `diff` и её приёмка письменного пояснения; второй дороги о том же смысле здесь
#  не заводится — разойдясь, они дали бы человеку два разных ответа на одно пояснение).
#  Будь у решения запись, рядом с правилом «нет даты или пробега — работа НЕ пишется» завёлся бы
#  второй путь в историю мимо самого правила; будь у него сеть — оно спросило бы записанное число
#  САМО, и «ниже ли записанного» поехало бы за тем, КАК спросили.
#  FAIL-CLOSED: файла нет / не парсится → ФЛАГ: недоказанная чистота доверия не имеет.
# --------------------------------------------------------------------------------------------
_BATCH_ODO_PATH = None          # подменяется САМОТЕСТОМ; None → боевой batch_odo.py в репо


@register("BATCH_ODO_PURE")
def check_batch_odo_pure(world, run):
    path = _BATCH_ODO_PATH or os.path.join(REPO, "batch_odo.py")
    try:
        with open(path, encoding="utf-8") as f:
            src = f.read()
    except OSError as e:
        run.flag("batch_odo.py", f"решение партии на другом пробеге не читается ({e}) — "
                                 f"чистота не доказана")
        return
    try:
        findings = _duty_ast_findings(src, allowed=frozenset(("re", "datetime", "odo_lower")))
    except SyntaxError as e:
        run.flag("batch_odo.py", f"решение партии на другом пробеге не разбирается ({e}) — "
                                 f"чистота не доказана")
        return
    for where, why in findings:
        run.flag(f"batch_odo.py:{where}", why)


#  --------------------------------------------------------------------------------------------
#  ИНВАРИАНТ 12ъ: UNDO_EXEC_PURE
#  Ответ двери отмены человеку (undo_exec.py, 26.08.2026) решает, ЧТО сказать о том, что дверь
#  уже ответила. Импорт РОВНО ОДИН — `reply_floor`, и берётся ВОКАБУЛЯР, а не руки: «мост вынес
#  отказ САМ или это молчание транспорта» он уже решает, и второго списка о том же смысле здесь
#  не заводится. Будь у решения вызов двери, «что сказать» поехало бы за тем, КАК спросили; будь
#  у него отправка — слова о возврате зависели бы от того, дошло ли сообщение. Отдельно важно,
#  что у него нет ЗАПИСИ: модуль стоит на пути единственной ветки splinter, которая правит живую
#  таблицу, и вторая рука к той же клетке рядом с судьёй её ответа — ровно тот класс, ради
#  которого решение и руки здесь разведены.
#  FAIL-CLOSED: файла нет / не парсится → ФЛАГ: недоказанная чистота доверия не имеет.
# --------------------------------------------------------------------------------------------
_UNDO_EXEC_PATH = None          # подменяется САМОТЕСТОМ; None → боевой undo_exec.py в репо


@register("UNDO_EXEC_PURE")
def check_undo_exec_pure(world, run):
    path = _UNDO_EXEC_PATH or os.path.join(REPO, "undo_exec.py")
    try:
        with open(path, encoding="utf-8") as f:
            src = f.read()
    except OSError as e:
        run.flag("undo_exec.py", f"ответ двери отмены не читается ({e}) — чистота не доказана")
        return
    try:
        findings = _duty_ast_findings(src, allowed=frozenset(("reply_floor",)))
    except SyntaxError as e:
        run.flag("undo_exec.py", f"ответ двери отмены не разбирается ({e}) — чистота не доказана")
        return
    for where, why in findings:
        run.flag(f"undo_exec.py:{where}", why)


# --------------------------------------------------------------------------------------------
@register("REPLY_FLOOR_PURE")
def check_reply_floor_pure(world, run):
    path = _REPLY_FLOOR_PATH or os.path.join(REPO, "reply_floor.py")
    try:
        with open(path, encoding="utf-8") as f:
            src = f.read()
    except OSError as e:
        run.flag("reply_floor.py", f"пол ответа не читается ({e}) — чистота не доказана")
        return
    try:
        findings = _duty_ast_findings(src, allowed=frozenset(("write_fact",)))
    except SyntaxError as e:
        run.flag("reply_floor.py", f"пол ответа не разбирается ({e}) — чистота не доказана")
        return
    for where, why in findings:
        run.flag(f"reply_floor.py:{where}", why)
...
# --------------------------------------------------------------------------------------------
#  ИНВАРИАНТ 12р: MONEY_AMOUNT_PURE
#  Решение о сумме движения (money_amount.py) отвечает на один вопрос: НАЗВАНА ли сумма числом,
#  или на её месте сейчас подставится ноль. Слепота к миру здесь и есть предмет: появись у
#  решения сеть — оно спросило бы баланс или лист САМО, и «сумма названа» стало бы зависеть от
#  того, ЧТО ответило хранилище, тогда как весь класс ровно про то, что хранилище о сумме не
#  спрашивают вовсе; появись файл — оно подтянуло бы «последнюю известную» сумму, то есть
#  завело бы вторую подстановку вместо закрытой первой.
#  Импорт РОВНО ОДИН — `balance_fact` (у него свой один, `write_fact`): различитель «это число?»
#  берётся готовым, второго способа сказать то же самое не заводится. Разбор — тот же ast: имя в
#  комментарии, строке и докстринге кодом не является (модуль о мосте и листе ГОВОРИТ, и это
#  законно — он называет, ГДЕ подставился бы ноль).
#  FAIL-CLOSED: файла нет / не парсится → ФЛАГ: недоказанная чистота доверия не имеет.
# --------------------------------------------------------------------------------------------
_MONEY_AMOUNT_PATH = None       # подменяется САМОТЕСТОМ; None → боевой money_amount.py в репо


@register("MONEY_AMOUNT_PURE")
def check_money_amount_pure(world, run):
    path = _MONEY_AMOUNT_PATH or os.path.join(REPO, "money_amount.py")
    try:
        with open(path, encoding="utf-8") as f:
            src = f.read()
    except OSError as e:
        run.flag("money_amount.py", f"решение о сумме не читается ({e}) — чистота не доказана")
        return
    try:
        findings = _duty_ast_findings(src, allowed=frozenset(("balance_fact",)))
    except SyntaxError as e:
        run.flag("money_amount.py", f"решение о сумме не разбирается ({e}) — чистота не доказана")
        return
    for where, why in findings:
        run.flag(f"money_amount.py:{where}", why)


# --------------------------------------------------------------------------------------------
#  ИНВАРИАНТ 12ч: CARD_WORKS_PURE
#  Список «в работе» (card_works.py, 25.08.2026) решает, что Пым увидит ждущим записи и, значит,
#  что он подтвердит кнопкой. Ошибки разнонаправлены: лишняя позиция стоит одного взгляда, а
#  молча снятая с ожидания работа не запишется уже никогда. Поэтому «оно только решает» держится
#  устройством: ИМПОРТОВ РОВНО НОЛЬ — ни строки парка, ни истории байка, ни классификатора видов,
#  ни контентного ключа модуль достать не может, всё приносят руки (`splinter._build_bike_card_body`),
#  а правила приходят параметрами. Будь у него сеть, «записано ли» снова зависело бы от того, КАК
#  спросили; будь у него запись — рядом с карточкой завёлся бы второй путь в заявку и в Лист1.
#  FAIL-CLOSED: файла нет / не парсится → ФЛАГ: недоказанная чистота доверия не имеет.
# --------------------------------------------------------------------------------------------
_CARD_WORKS_PATH = None         # подменяется САМОТЕСТОМ; None → боевой card_works.py в репо


@register("CARD_WORKS_PURE")
def check_card_works_pure(world, run):
    path = _CARD_WORKS_PATH or os.path.join(REPO, "card_works.py")
    try:
        with open(path, encoding="utf-8") as f:
            src = f.read()
    except OSError as e:
        run.flag("card_works.py", f"решение списка «в работе» не читается ({e}) — чистота не доказана")
        return
    try:
        findings = _duty_ast_findings(src, allowed=frozenset())
    except SyntaxError as e:
        run.flag("card_works.py", f"решение списка «в работе» не разбирается ({e}) — чистота не доказана")
        return
    for where, why in findings:
        run.flag(f"card_works.py:{where}", why)


# --------------------------------------------------------------------------------------------
#  ИНВАРИАНТ 12ц: SERVICE_DEBT_PURE
#  Долг принятой работы (service_debt.py, 23.08.2026) решает СУДЬБУ СЛЕДА: висит работа долгом,
#  закрыта доказанным признаком или исход неизвестен. Ошибки разнонаправлены и обе дороги —
#  закрытый зря долг исчезает МОЛЧА (ровно тот класс, ради которого модуль писан: 01.08 принято 2,
#  записано 0, и никто не сказал ни слова), а не закрытый зря шумит владельцу. Поэтому «оно только
#  решает» держится устройством: ИМПОРТОВ РОВНО НОЛЬ — ни расписки, ни клетки, ни «сейчас», ни
#  порогов модуль достать не может, всё приносят руки (`splinter._sp_debt_*`). Будь у него сеть,
#  «записано ли» снова зависело бы от того, КАК спросили, а не от ответа мира; будь у него
#  запись — рядом с долгом завёлся бы второй путь в Лист1 мимо «да» доверенного.
#  FAIL-CLOSED: файла нет / не парсится → ФЛАГ: недоказанная чистота доверия не имеет.
# --------------------------------------------------------------------------------------------
_SERVICE_DEBT_PATH = None       # подменяется САМОТЕСТОМ; None → боевой service_debt.py в репо


@register("SERVICE_DEBT_PURE")
def check_service_debt_pure(world, run):
    path = _SERVICE_DEBT_PATH or os.path.join(REPO, "service_debt.py")
    try:
        with open(path, encoding="utf-8") as f:
            src = f.read()
    except OSError as e:
        run.flag("service_debt.py", f"решение долга не читается ({e}) — чистота не доказана")
        return
    try:
        findings = _duty_ast_findings(src, allowed=frozenset())
    except SyntaxError as e:
        run.flag("service_debt.py", f"решение долга не разбирается ({e}) — чистота не доказана")
        return
    for where, why in findings:
        run.flag(f"service_debt.py:{where}", why)
...
# --------------------------------------------------------------------------------------------
#  ИНВАРИАНТ 12п: DELIVER_CARD_PURE
#  Решение «предложить ли владельцу доставку коммита» (deliver_card.py, путь C цели 538) стоит
#  ближе всего к операции из всех чистых модулей: за его карточкой стоит ПЕРЕЗАПУСК живого
#  процесса. Тем важнее, чтобы вызвать эту операцию ему было НЕЧЕМ: импорт ровно один
#  (`expectations` — вокабуляр прибора берётся готовым, чтобы своего суждения о доставке здесь
#  не завелось), ни файлов, ни сети, ни моста, ни очереди, ни подпроцессов. Тогда худшее, что
#  ошибка в его логике может стоить, — лишний ВОПРОС владельцу; исполнить он не может ничего.
#  FAIL-CLOSED: файла нет / не парсится → ФЛАГ: нечитаемое правило доверия не имеет.
# --------------------------------------------------------------------------------------------
_DELIVER_CARD_PATH = None       # подменяется САМОТЕСТОМ; None → боевой deliver_card.py в репо


@register("DELIVER_CARD_PURE")
def check_deliver_card_pure(world, run):
    path = _DELIVER_CARD_PATH or os.path.join(REPO, "deliver_card.py")
    try:
        with open(path, encoding="utf-8") as f:
            src = f.read()
    except OSError as e:
        run.flag("deliver_card.py", f"решение доставки не читается ({e}) — чистота не доказана")
        return
    try:
        findings = _duty_ast_findings(src, allowed=frozenset(("expectations",)))
    except SyntaxError as e:
        run.flag("deliver_card.py", f"решение доставки не разбирается ({e}) — чистота не доказана")
        return
    for where, why in findings:
        run.flag(f"deliver_card.py:{where}", why)


# --------------------------------------------------------------------------------------------
#  ИНВАРИАНТ 12р: RESULT_REF_PURE
#  Адрес результата шага (result_ref.py, пункт 1 контракта третьего исхода) — ФОРМА поля
#  очереди и ничего сверх неё: где обязан появиться продукт шага. Главная защита этого захода
#  в том, что связь не может протянуться раньше времени: чтобы СХОДИТЬ по названному адресу
#  (в git, на диск, в базу, в мозг, в /proc), решению нужны руки — и их у него нет. Импорт
#  ровно один (`re`), ни файлов, ни сети, ни подпроцессов, ни моста. Пока стоит этот страж,
#  «поле инертно» — свойство устройства, а не обещание в отчёте; проверка по адресу заведётся
#  отдельным заходом и в отдельном месте, у которого руки есть законно.
#  FAIL-CLOSED: файла нет / не парсится → ФЛАГ: недоказанная чистота доверия не имеет.
# --------------------------------------------------------------------------------------------
_RESULT_REF_PATH = None         # подменяется САМОТЕСТОМ; None → боевой result_ref.py в репо


@register("RESULT_REF_PURE")
def check_result_ref_pure(world, run):
    path = _RESULT_REF_PATH or os.path.join(REPO, "result_ref.py")
    try:
        with open(path, encoding="utf-8") as f:
            src = f.read()
    except OSError as e:
        run.flag("result_ref.py", f"форма адреса результата не читается ({e}) — чистота не доказана")
        return
    try:
        findings = _duty_ast_findings(src, allowed=frozenset(("re",)))
    except SyntaxError as e:
        run.flag("result_ref.py", f"форма адреса результата не разбирается ({e}) — чистота не доказана")
        return
    for where, why in findings:
        run.flag(f"result_ref.py:{where}", why)


# --------------------------------------------------------------------------------------------
#  ИНВАРИАНТ 12с: RESULT_JUDGE_PURE
#  Судья адреса результата (result_judge.py, пункт 2 контракта третьего исхода) отвечает, лежит
#  ли по названному адресу продукт шага: ДОКАЗАН · НЕ ДОКАЗАН · НЕИЗВЕСТНО. Опасность у него
#  ровно одна и обратная гардовой: не «исполнит лишнее», а «скажет зелёное, ничего не прочитав».
#  Поэтому руки у судьи отняты устройством: импорт ровно один — `result_ref` (вокабуляр адреса
#  берётся ГОТОВЫМ, чтобы второе определение формы не завелось рядом; тот же приём, каким
#  `deliver_card` берёт вокабуляр у `expectations`), ни файлов, ни сети, ни базы, ни
#  подпроцессов. Всё, что трогает мир, живёт отдельно — `result_judge_facts.py`, и приносит
#  судье ФАКТЫ с честной пометкой «не прочитано».
#  FAIL-CLOSED: файла нет / не парсится → ФЛАГ: недоказанная чистота доверия не имеет.
# --------------------------------------------------------------------------------------------
_RESULT_JUDGE_PATH = None       # подменяется САМОТЕСТОМ; None → боевой result_judge.py в репо


@register("RESULT_JUDGE_PURE")
def check_result_judge_pure(world, run):
    path = _RESULT_JUDGE_PATH or os.path.join(REPO, "result_judge.py")
    try:
        with open(path, encoding="utf-8") as f:
            src = f.read()
    except OSError as e:
        run.flag("result_judge.py", f"судья адреса не читается ({e}) — чистота не доказана")
        return
    try:
        findings = _duty_ast_findings(src, allowed=frozenset(("result_ref",)))
    except SyntaxError as e:
        run.flag("result_judge.py", f"судья адреса не разбирается ({e}) — чистота не доказана")
        return
    for where, why in findings:
        run.flag(f"result_judge.py:{where}", why)


# --------------------------------------------------------------------------------------------
#  ИНВАРИАНТ 12т: SHADOW_RULE_PURE
#  Теневой прогон правила зелёного (shadow_rule.py, пункт 3 контракта третьего исхода в
#  БЕЗОПАСНОЙ форме) отвечает, каким БЫЛ БЫ вердикт шага, будь правило включено. Опасность у
#  него одна и названа прямо: правило, протянувшееся раньше времени, остановит полосу — адрес
#  сегодня не пишет ни один боевой вызов, поэтому включённое правило сделало бы КАЖДЫЙ шаг
#  незелёным. «Тень ничего не решает» обязано держаться устройством: импорт ровно один —
#  `result_judge` (вокабуляр исходов и порядок силы берутся ГОТОВЫМИ, чтобы второе определение
#  не завелось рядом), ни файлов, ни сети, ни подпроцессов, ни очереди. Закрыть задачу, двинуть
#  цепь или тронуть счёт серии этой функции нечем ФИЗИЧЕСКИ.
#  FAIL-CLOSED: файла нет / не парсится → ФЛАГ: недоказанная чистота доверия не имеет.
# --------------------------------------------------------------------------------------------
...
def check_shadow_rule_pure(world, run):
    path = _SHADOW_RULE_PATH or os.path.join(REPO, "shadow_rule.py")
    try:
        with open(path, encoding="utf-8") as f:
            src = f.read()
    except OSError as e:
        run.flag("shadow_rule.py", f"теневое правило не читается ({e}) — чистота не доказана")
        return
    try:
        findings = _duty_ast_findings(src, allowed=frozenset(("result_judge",)))
    except SyntaxError as e:
        run.flag("shadow_rule.py", f"теневое правило не разбирается ({e}) — чистота не доказана")
        return
    for where, why in findings:
        run.flag(f"shadow_rule.py:{where}", why)


# --------------------------------------------------------------------------------------------
#  ИНВАРИАНТ 12щ: WA_KIND_PURE
#  Вид записи входящей очереди WhatsApp (wa_kind.py, 07.09.2026) решает, чем является строка,
#  которую забор отдаёт ПК: сообщением клиента, эхом менеджера, историей, квитанцией доставки —
#  или неопознанным. От этого решения зависит, поднимется ли карточка живому человеку. Импортов
#  РОВНО НОЛЬ: все три различителя (`echo`, `history`, `msg_type`) приносит вызывающий из СВОЕЙ
#  строки. Будь у решения база — оно спросило бы очередь само, и вид зависел бы от того, КАК
#  спросили, а не от строки, которую выдают; будь у него сеть — рядом с решением завёлся бы второй
#  путь наружу, к живому номеру клиента, мимо закрытой отправляющей двери; будь у него часы —
#  «история» поехала бы за живым временем, и голдены перестали бы задевать свою ветку.
#  FAIL-CLOSED: файла нет / не парсится → ФЛАГ: недоказанная чистота доверия не имеет.
# --------------------------------------------------------------------------------------------
_WA_KIND_PATH = None            # подменяется САМОТЕСТОМ; None → боевой wa_kind.py в репо


@register("WA_KIND_PURE")
def check_wa_kind_pure(world, run):
    path = _WA_KIND_PATH or os.path.join(REPO, "wa_kind.py")
    try:
        with open(path, encoding="utf-8") as f:
            src = f.read()
    except OSError as e:
        run.flag("wa_kind.py", f"решение о виде записи не читается ({e}) — чистота не доказана")
        return
    try:
        findings = _duty_ast_findings(src, allowed=frozenset())
    except SyntaxError as e:
        run.flag("wa_kind.py",
                 f"решение о виде записи не разбирается ({e}) — чистота не доказана")
        return
    for where, why in findings:
        run.flag(f"wa_kind.py:{where}", why)
